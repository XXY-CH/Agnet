...
This draft describes the local-first prototype at `v12.12-protocol`. It is not a full Agent Space product spec.
...
For `proof-bundle`, the signed receipt transport proof MUST be an object and MUST carry non-empty `transport`, non-empty `listen_host`, decimal-string `port`, and `public_transport: true`. This is local listener evidence, not a remote reachability proof from another host.
