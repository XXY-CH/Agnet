...
This draft describes the local-first prototype at `v11.13-protocol`. It is not a full Agent Space product spec.
...
The implemented Node verifier checks the signing Zone, the close signature, the signed Swarm id, the frame/body Swarm id match, and the structure of `step_receipts`. It requires at least one step receipt, requires each step receipt to include `step_id`, `task_id`, and a 64-hex `receipt_digest`, and rejects duplicate `step_id` values.
