...
This draft describes the local-first prototype at `v12.16-protocol`. It is not a full Agent Space product spec.
...
The `proof-bundle` verifier reports `reachability_scope: "local-interface"` for the implemented public proof bundle. This is verifier-owned labeling of the current proof scope, not external-host reachability.
