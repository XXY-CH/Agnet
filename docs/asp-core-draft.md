...
- When receipt checkpoint evidence is present, Node `FED_RECEIPT` verification requires `checkpoint_refs` and `checkpoints` to have equal length, each checkpoint to bind the receipt `task_id`, each checkpoint id to match the corresponding ref, the parent chain to start at `receipt.resumed_from` or `null`, and the worker `checkpoint_signature` to verify.
...
node scripts/external-reachability-observer.mjs <bundle.json> <observed-bundle.json> <observer-trusted-zones.json> <container|cross-netns|external-host>
bash scripts/docker-external-reachability-observer.sh <bundle.json> <observed-bundle.json> <observer-trusted-zones.json>
bash scripts/container-cross-netns-observer.sh <bundle.json> <observed-bundle.json> <observer-trusted-zones.json>
```
...
The `proof-bundle` verifier reports `reachability_scope: "local-interface"` when no external reachability evidence is supplied and rejects bundle manifests that supply their own `reachability_scope`. With trusted observer evidence, it reports `reachability_scope: "container-observer"` for `vantage: "container"`, `reachability_scope: "cross-netns"` for `vantage: "cross-netns"` only when the signed receipt `listen_host` is a literal private inter-namespace IP, or `reachability_scope: "external-host"` for `vantage: "external-host"` only when the signed receipt `listen_host` is a globally routable literal IP. Hostname listen hosts are out of scope for `cross-netns` and `external-host` in this slice. Observer-backed scopes return `reachability_observer_zid`.

When `proof-bundle` receives an additional caller-supplied trusted-Zone file, it MAY accept `bundle.external_reachability` evidence signed by a trusted observer Zone. The evidence MUST bind `proof: "external-reachability"`, `observer_zid`, `vantage`, `observed_host`, `observed_port`, `observed_at`, `transport_proof`, `receipt_digest`, and `reached: true`; the verifier rejects stale, future, wrong-endpoint, wrong-digest, invalid-vantage, untrusted-observer, unsigned, non-private cross-netns listen hosts, and non-routable external-host evidence. A bundle that carries external reachability evidence without the extra trust input MUST fail closed. This is a verifier-owned evidence gate, not hosted-node deployment, NAT traversal, or completion of the real hosted external-host observer run.

`scripts/external-reachability-observer.mjs` is the minimal implemented observer writer for that evidence shape. It reads a proof bundle, verifies the bundle's signed receipt digest and transport proof match, TCP-connects to `transport_proof.listen_host:port`, records the requested vantage plus observed endpoint and freshness, and writes an observed bundle plus observer trusted-Zone file. `scripts/docker-external-reachability-observer.sh` runs the same observer from a Docker container using Docker's host gateway and `${AGNET_NODE_BASE_IMAGE:-node:22-bookworm-slim}` with container vantage; it proves a container boundary, not hosted deployment. `scripts/container-cross-netns-observer.sh` runs the observer from Apple `container`'s separate-VM model with `${AGNET_NODE_BASE_IMAGE:-node:24-alpine}` and `cross-netns` vantage. The cross-netns verifier path requires a literal private IP such as `192.168.64.6`, rejects loopback and globally routable IPs, and proves only private cross-network-namespace reachability, not public or hosted external-host reachability.
...
`FED_QUERY` may carry an `intent` string for semantic discovery and may carry task `scope` constraints for policy-aware ranking. Ranking is deterministic and evidence-first: exact capability match, trusted capability credential, signed credential claims, audit-backed reputation, zone-chain provenance, verifier-owned routing signals, and semantic token overlap are exposed as inspectable evidence. The Go federation gateway FED_QUERY accepts `intent` and returns ranked matches with `discovery_evidence` and `ranking` fields, mirroring the Node surface. For reputation, `discovery_evidence.reputation` exposes `completed_receipts`, `last_completed_at`, `revocation_count`, and labelled `agent_score` components: `receipt_score`, `credential_score`, `freshness_score`, `cost_score`, `latency_score`, `availability_score`, `policy_match`, `risk_match`, and `revocation_penalty`. `discovery_evidence.zone_trust_chain` carries signed cross-zone delegation provenance when a match arrives through delegated trust. `ranking.score` is `agent_score.total` plus exact capability and semantic intent scores. receipt counts come from the persisted audit log; this is not a hardcoded demo value, not cross-session learned scoring, and not a third-party reputation service.

In the v14.7 routing surface, `discovery_evidence.routing` exposes `cost_score`, `latency_score`, `availability_score`, `policy_match`, `risk_match`, and `signals_used`. `cost_score` comes from descriptor `policy.cost_tokens_per_task`, `latency_score` comes from descriptor `policy.latency_ms_p95`, and `availability_score` comes from local audit evidence over the last 50 entries. `policy_match` comes only from existing descriptor policy and query/task scope fields such as network and write-prefix allowances. `risk_match` comes only from existing active credential trust, local signed revocation state, and completed-receipt reputation. Missing signal evidence falls back to neutral score `5` and does not increment `signals_used`; `agent_score.total` includes the routing scores and remains capped at 100.
...
authority Zone revocation in FED_QUERY discovery makes signed capability credentials inactive for revoked workers: revoked workers still expose trusted credential history when a signature exists, but `discovery_evidence.credential.active` becomes `false`, and revoked workers lose credential contribution in `agent_score`. This is local authority Zone evidence checked by Node and Go query ranking, not network revocation sync, not a distributed registry, and not a third-party service.
