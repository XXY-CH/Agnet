...
This draft describes the local-first prototype at `v11.17-protocol`. It is not a full Agent Space product spec.
...
The implemented Node verifier checks the signing Zone, close proof object, close signature presence and verification, the signed Swarm id, the frame/body Swarm id match, and the structure of `step_receipts`. It requires at least one step receipt, requires each step receipt to include `step_id`, a safe `task_id` token, and a 64-hex `receipt_digest`, and rejects duplicate or NUL-bearing Swarm identities.
