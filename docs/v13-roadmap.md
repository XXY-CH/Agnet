...
状态：planned
目标：把 v12 的 local-interface proof 推进到 real external-host evidence backed by a hosted node.

验收边界：

- A hosted Go federation gateway is reachable from an independently run observer outside the host/network namespace of the server.
- The observer evidence is signed and bound to the same receipt digest and transport proof that `proof-bundle` verifies.
- The proof distinguishes `local-interface`, container observer, and real `external-host` reachability without accepting bundle-owned labels.
- Negative tests reject stale, wrong-host, wrong-port, wrong-digest, untrusted-observer, and unsigned reachability evidence.
