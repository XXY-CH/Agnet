...
- `Dockerfile` and `scripts/docker-proof-demo.sh` define a Docker proof demo contract; local Docker daemon verification is still pending。
...
- Go `FED_SWARM_CLOSE` carries one ordered complete audit-backed Zone-signed close proof over ordered step receipt digests。
...
- Explicit Swarm seed executes two ordered Go worker steps, signs the downstream artifact dependency into the receipt, appends one close proof to audit, and verifies complete close proof step order and digests against same-audit receipts。
