...
- Go `FED_SWARM_CLOSE` carries Zone-signed close proof over ordered step receipt digests。
...
- Explicit Swarm seed executes two ordered Go worker steps, signs the downstream artifact dependency into the receipt, and signs a close proof over ordered step receipt digests。
