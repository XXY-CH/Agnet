...
它已经抓住了真正 Agent Net 的底层骨架：identity、signed task、policy、artifact、receipt、audit、federation、Node/Go bidirectional task interop、Go federation explicit bind host primitive、shared `FED_TASK_OPEN` / `FED_RECEIPT` conformance fixtures，以及 Go 侧最小 explicit Swarm DAG seed 和同 audit Swarm dependency verification。
...
- Go audit verification rejects signed Swarm dependency claims that do not match upstream step artifact manifests in the same audit。
...
- Audit verification checks Swarm dependency artifact manifest claims against prior completed steps in the same audit。
