...
- Go `FED_SWARM_CLOSE` carries audit-backed Zone-signed close proof over ordered step receipt digests。
...
- Explicit Swarm seed executes two ordered Go worker steps, signs the downstream artifact dependency into the receipt, appends a close proof to audit, and verifies close proof step digests against same-audit receipts。
