...
**AFP transition note (2026-07-22):** v14 remains the implemented ASP local-first evidence line and its historical non-claims remain binding. The next architecture program is AFP v1, documented in `docs/afp-v1-design.md`; it does not relabel v14 frames as AFP, claim that P2P or settlement is implemented, or weaken this roadmap's local-first proof boundary.
...
- `test/docs-contract.test.mjs` guards the v14 opening wording and non-claims.
...
状态：complete
目标：Bring the Node federation gateway to parity with the existing Go scheduler-owned ready-DAG execution contract.

新增：

- Node accepts `FED_SWARM_SCHEDULE`, validates the entire graph before any step execution, and executes dependency-ready steps serially.
- Ready-step ties use original input order, matching Go `scheduleSwarmSteps`.
- The signed close carries exactly `scheduler: { mode: "ready-dag", step_order: [...] }`; normal `FED_SWARM_OPEN` remains input-order and carries no scheduler field.
- Node `verifySwarmClose` optionally validates `ready-dag` scheduler evidence and requires `step_order` to match ordered close `step_receipts` position by position.
- Missing dependencies, duplicate step IDs, self-dependencies, and a cycle or unresolvable graph fail before any receipt or close is emitted.
不做：
...
- A reverse dependency-order schedule executes dependency-first and signs the executed `step_order`.
- A cycle or unresolvable graph returns `FED_TASK_ERROR` before `FED_RECEIPT` or `FED_SWARM_CLOSE`.
- Existing signed task verification, dependency artifact/receipt binding, micro-contracts, one-step migration, conflict resolutions, plan digest propagation, signed close, and audit continuity stay in force.

## v14.11 / Phase C: Local foundation closure

状态：complete for the local proof kernel

v14.11 completes the Darwin Apple private-workspace isolation proof. Phase C U19-U30 completes the Go-local durable Swarm foundation: a same-host filesystem journal under OS process locks is authoritative, materializes replayable views, records deterministic parallel ready waves, emits a byte-stable close, gates irreversible signed disband on output verification, and keeps worker execution at-least-once while the fenced signed receipt commitment is exactly-once.

Node is a pure verifier of fixed offline U29 vectors for this durable format. Live public proof excludes durable Swarm completion. These closure claims do not assert globally distributed operation, public durable-Swarm completion, real Docker smoke, hardware attestation, cross-host operation, remote artifact handling, or exactly-once worker execution. `FED_SWARM_SCHEDULE` in Node remains serial ready-DAG execution; it is not the Phase C parallel-ready-wave runtime.
验收：

- U1-U30 local-swarm-foundations are complete.
- Node suite: 531/531; Go gates pass.
- The protocol records local transport, task, receipt, artifact, and reachability proof separately from durable Swarm proof, which is offline-vector-only on Node.
