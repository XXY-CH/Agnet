...
- FED_QUERY results expose cost estimate, latency estimate, and availability evidence as labelled ranking components.
...
- A trust delegation chain binds delegator Zone, delegated Zone or worker, allowed capability, validity window, and revocation boundary.
- Verification accepts a worker only when the chain starts from a trusted local root and every delegation signature verifies.
- FED_RESOLVE/FED_QUERY/FED_TASK_OPEN can carry enough chain evidence for local verification.
...
- Negative tests reject broken signature, expired delegation, wrong capability, wrong subject, and untrusted root.
- Direct trust store behavior remains compatible for local-first deployments.

## v14.4: Task failure migration

状态：planned
目标：worker failure triggers re-assignment to next-best candidate while preserving signed failure evidence and Swarm audit continuity.

新增：

- Swarm step failure records a signed failure receipt or migration event.
- Scheduler chooses the next-best eligible candidate using the same evidence-ranked candidate set.
- The migrated step binds prior failure evidence, replacement worker, retry lineage, and artifact/checkpoint inputs.
- FED_SWARM_CLOSE records both failed attempt evidence and final successful step receipt.
...
- A worker failure causes re-assignment to a next-best candidate and preserves failure evidence.
- Tests prove close verification rejects migrated closes missing failure lineage or replacement receipt binding.
- Migration does not hide the original failed worker or mutate prior receipts.
