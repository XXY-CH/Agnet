...
状态：v13 active at v13.10
当前代码基线：`v13.10-go-fed-query-semantic-parity`
...
v13.10 Go FED_QUERY semantic parity is complete: `cmd/go-fed-discovery/main.go` now accepts `intent`, calls `queryMatch`, ranks matches by exact capability, trusted credential evidence, receipt-count input, and semantic token overlap, and returns `discovery_evidence` plus `ranking.score`/`ranking.reasons` for each candidate.
...
The active protocol tag is `v13.10-protocol`: Go FED_QUERY semantic discovery parity is the latest complete slice, while the v13.1 hosted external-host observer run and hardware remote attestation remain pending exit criteria.
...
| Capability query | done + Node federation gateway evidence-first semantic discovery/reputation ranking | done + Go FED_QUERY semantic intent scoring, evidence-first ranking, discovery_evidence, and ranking.score/reasons | `federation-gateway.test.mjs`, `go-fed-discovery.test.mjs` | scheduling |
