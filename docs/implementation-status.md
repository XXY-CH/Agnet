...
状态：v12.39 active
当前代码基线：`v12.39-package-proof-trusted-signer-pin`
...
Node
  ...
  -> asp-verify.mjs package-proof <manifest.json> verifies package proof_digest, ASP package proof signature, optional trusted signer pin, manifest-relative tarball sha256, npm shasum, npm integrity, filename/tarball basename binding, manifest/input filename binding, package identity filename binding, packaged file list shape, and manifest-relative tarball size, returns verified package metadata including signer_aid and signer_trusted, rejects non-object package proof manifests, and rejects unsafe package proof tarball paths
...
v12.39 adds optional trusted signer pinning for local package proof verification. `asp-verify.mjs package-proof <manifest.json> [trusted-signers.json]` accepts `{ "signers": [...] }` or a raw descriptor array, validates those signer descriptors through the existing Agent descriptor path, and returns `signer_trusted: true` only when the package proof signer `aid` appears in that caller-supplied trusted set. This is trusted-signer pinning for local package proofs, not npm registry signing, release transparency, SBOM, package publish, hosted reachability, scheduler ownership, batch verification, or A2A/ARD compatibility.
