...
| `FED_SWARM_OPEN` | not yet | explicit ordered two-step DAG seed; each step reuses signed task execution; receipts bind dependency step ids, artifact manifests, and upstream receipt digest; audit verifier rejects duplicate step receipts and false signed dependency steps, manifests, or receipt digests | `go-fed-discovery.test.mjs`, `cmd/go-fed-discovery/main_test.go` | dynamic decomposition / scheduler / parallel execution / conflict resolution / cross-Zone Swarm |
...
Go
  ...
  -> verify-audit checks Swarm input_artifacts against declared after steps, unique upstream step ids, upstream step artifact manifests, and receipt digest in the same audit
...
v9.30 rejects duplicate Swarm step receipts in the same audit. The next natural boundary should either deepen Swarm orchestration without building a scheduler too early, align Node artifact manifests with Go, or start a real container namespace sandbox proof.
