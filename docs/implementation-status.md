...
| Receipt signing | done + single FED_RECEIPT frame verification CLI | done for minimal Go execution + single receipt record verification CLI | `test-vectors.test.mjs`, `go-fed-discovery.test.mjs` | receipt store/search / batch verification |
...
v10.5 is closed. The next natural boundary should keep making the accountability layer easier to verify externally: package receipt verification, publish a narrow ASP Core draft, or provide a reproducible public-node/Docker demo.
