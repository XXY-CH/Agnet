...
| Product surface | CLI/tests only | thin Human Gateway with task table, queue table, and scoped signed explicit local enqueue/claim/drain actions | README, `go-fed-discovery.test.mjs` | approvals, task drafting/signing, admin, deployment |
...
Go
  ...
  -> Human Gateway /api/queue and scoped signed explicit queue enqueue/claim/drain actions
...
```text
v7.11 Queue Action Replay Boundary
  -> prevent grant replay before richer product UI
```
