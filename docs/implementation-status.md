...
状态：v13 active at v13.13
当前代码基线：`v13.13-zone-revocation-discovery`
...
v13.13 zone revocation discovery is complete: authority Zone revocation in FED_QUERY discovery sets `discovery_evidence.credential.active: false`; revoked workers get no credential score boost, while non-revoked workers keep the existing active credential behavior.
...
v13.11 audit-backed receipt-count reputation is complete: Node and Go semantic discovery now count completed receipts for each worker from the persisted audit log before ranking query results. receipt counts come from the persisted audit log; this is not a hardcoded demo value, not cross-session ML, not a global reputation oracle.
...
The active protocol tag is `v13.13-protocol`: authority Zone revocation in FED_QUERY discovery is the latest complete slice, while the v13.1 hosted external-host observer run and hardware remote attestation remain pending exit criteria.
...
| Capability query | done + Node federation gateway evidence-first semantic discovery/reputation ranking with persisted audit-backed receipt counts and authority Zone revocation in FED_QUERY discovery | done + Go FED_QUERY semantic intent scoring, evidence-first ranking, discovery_evidence, ranking.score/reasons, persisted audit-backed receipt counts, and fixture-backed authority Zone revocation in FED_QUERY discovery | `federation-gateway.test.mjs`, `go-fed-discovery.test.mjs` | scheduling |
| Capability credential | done + revoked workers get no credential score boost in discovery | done + Go signed status evidence + revoked workers get no credential score boost in discovery | `capability-credential.test.mjs`, `federation-gateway.test.mjs`, `go-fed-discovery.test.mjs` | renewal |
...
| `FED_SWARM_OPEN` / `FED_SWARM_SCHEDULE` | not yet | explicit ordered two-step DAG seed plus scheduler-owned ready-DAG execution for out-of-order signed DAG steps; each step reuses signed task execution; receipts bind delimiter-safe dependency step ids, artifact manifests, and upstream receipt digest; `FED_SWARM_CLOSE` carries one complete audit-backed Zone-signed ordered close proof over same-audit receipts and may include signed `ready-dag` scheduler step order; audit verifier rejects duplicate step receipts, including artifactless duplicates, NUL-bearing Swarm identities, false signed dependency steps/manifests/receipt digests, incomplete/duplicate/reordered close summaries, duplicate close records, close proofs for audit-absent Swarms, and false close step digests | `go-fed-discovery.test.mjs`, `cmd/go-fed-discovery/main_test.go` | automatic decomposition / parallel execution / conflict resolution / cross-Zone Swarm |
...
Node
  ...
  -> asp-verify.mjs proof-bundle <bundle.json> [external-trusted-zones.json] keeps reachability_scope verifier-owned, reports local-interface without observer evidence, upgrades to container-observer for trusted container vantage evidence, upgrades to external-host only for trusted external-host vantage evidence over a globally routable literal-IP listen_host, returns reachability_observer_zid for observer-backed scopes, and rejects stale/future/mismatched/untrusted/unsigned/bundle-owned/non-routable evidence
...
Go
  ...
  -> FED_SWARM_SCHEDULE scheduler-owned ready-DAG execution for out-of-order signed DAG steps
...
v13 is active at `v13.13-protocol`, with v13.13 authority Zone revocation in FED_QUERY discovery as the latest complete slice. authority Zone revocation in FED_QUERY discovery sets `discovery_evidence.credential.active: false`; revoked workers get no credential score boost, while signed credential history remains inspectable. v13.12 credential validity window remains complete: Capability credentials may carry a `valid_until` ISO UTC expiry in claims; expired credentials lower discovery score and report `active: false` in discovery evidence. v13.11 audit-backed receipt-count reputation remains complete: receipt counts come from the persisted audit log; this is not a hardcoded demo value, not cross-session ML, not a global reputation oracle. v13.1 reachability evidence gates remain active: verifier-owned scope classes and observer evidence binding landed with tests; real hosted external-host evidence is still pending as the remaining v13.1 exit criterion. The current hosted attempt is recorded as GitHub Actions run `28916288568`, which failed with `ENETUNREACH` against the current IPv6 listener.

The closed v12 proof surface remains in force; the current proof surface covers proof-bundle manifests, verifier-owned reachability scope classes (`local-interface`, `container-observer`, `external-host`), observer evidence binding to `vantage`, observed endpoint, freshness, transport proof, receipt digest, `reachability_observer_zid` for observer-backed scopes, pre-pinned observer identity through `AGNET_REACHABILITY_OBSERVER_SEED_HEX`, explicit public proof listen host and keepalive controls through `AGNET_PUBLIC_LISTEN_HOST` and `AGNET_PUBLIC_PROOF_KEEPALIVE_MS`, the Hosted Reachability Observer workflow, Docker observer wrappers, exact verifier CLI arity, local npm package proof manifests, package tarball byte verification, package proof digest/signature verification, package trusted signer pinning, metadata preflight, package signer capability binding, release trust producer output, `asp-release-trust/v1` release trust manifests, release trust verifier package proof binding, tarball byte binding, package proof digest stale detection, `released_at` shape validation, release signer capability binding, trusted release signer pinning, sandbox-proof <frame.json> <trusted-zones.json> [required-sandbox-class] verifier-owned local-process class validation, sandbox-attestation <frame.json> <trusted-zones.json> <attestation.json> <trusted-attestors.json> signed attestation evidence validation, Node federation gateway evidence-first semantic discovery/reputation ranking, Go FED_QUERY semantic intent scoring with discovery_evidence and ranking.score/reasons, and Go `FED_SWARM_SCHEDULE` scheduler-owned ready-DAG execution. `container-observer` is a distinct verifier-owned scope, not hosted proof; `external-host` requires a globally routable literal-IP listen host; hostname listen hosts are out of scope for this slice; the real hosted external-host observer run has not happened yet. Release trust/SBOM here is not CycloneDX, not SPDX, not SLSA provenance, not npm registry signing, not package publish, not release transparency, and not a generic supply-chain platform. Sandbox proof and attestation verification here are not hardware remote attestation, not container namespace execution, and not a VM/TEE claim. Semantic discovery/reputation ranking here is not a vector database, not global reputation, not a public marketplace, and not scheduler integration. Dynamic Swarm scheduling here is not automatic decomposition, not parallel execution, not upper-layer master-agent orchestration, and not economic settlement.

Route detail: `docs/v13-roadmap.md`。

Skipped until later: DID-native resolver/document/service routing, browser multi-key manager, requester selector UI, alias delete/disable, server-side rotation registry, encrypted key store, passphrase-protected export, public transport/QUIC, full login/session system, token rotation/storage, dynamic policy service, policy UI, distributed nonce service, external signer trust pin, long-running MCP sessions, container namespace sandbox, object-store artifact backend, Git/worktree/merge operations, scheduler queues, dynamic Swarm decomposition, parallel Swarm execution, A2A/ARD compatibility.
