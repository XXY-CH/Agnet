...
v11.46 makes Node and Go reusable receipt verifiers reject malformed artifact manifest `sha256` values before accepting signed receipt artifact metadata. This keeps artifact closure tied to real SHA-256 content addresses without changing artifact manifest metadata, remote artifact fetch, object-store backends, retention policy, or audit path behavior. The next natural boundary should keep moving toward Ultimate without widening the claim surface: close remaining Phase A trust-boundary bugs only when still present in live code, add real public reachability proof only with external network evidence, or continue Swarm proof work only where it adds verifiable accountability without scheduler breadth.
