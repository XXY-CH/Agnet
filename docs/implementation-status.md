...
| Reproducible demo | `scripts/proof-demo.sh` local proof demo + Docker proof demo contract | not yet | `proof-demo.test.mjs`, `docker-demo.test.mjs` | Docker image run proof / public node / hosted demo |
...
v10.10 is closed. The next natural boundary should keep making the accountability layer easier to verify externally: run and publish Docker proof output once a Docker daemon is available, or add npm-facing packaging only if the existing Node exports are not enough.
