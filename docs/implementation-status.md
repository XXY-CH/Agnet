...
Go
  ...
  -> local public-listen proof for authenticated resolve, query, task execution, and audit receipt query
...
v10.25 is closed. The next natural boundary should keep making the accountability layer easier to verify externally: make the public proof verifier-ready by emitting fetched receipt/trust files, continue Swarm proof only where it adds verifiable accountability, or add npm-facing packaging only if the existing Node exports are not enough.
