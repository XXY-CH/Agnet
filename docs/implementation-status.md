...
Go
  ...
  -> local public-listen proof for authenticated resolve, query, task execution, audit receipt query, artifact byte fetch, out-of-receipt artifact read rejection, verifier-ready receipt/trust files, and fetched artifact byte verification
...
v10.28 is closed. The next natural boundary should keep making the accountability layer easier to verify externally: add tamper-negative coverage for `FED_ARTIFACT_READ` when local artifact bytes drift after receipt creation, continue Swarm proof only where it adds verifiable accountability, or add npm-facing packaging only if the existing Node exports are not enough.
