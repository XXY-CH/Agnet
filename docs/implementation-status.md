...
| Reproducible demo | `scripts/proof-demo.sh` local proof demo emits verifier-ready receipt/trust files and verifies local artifact closure + `scripts/docker-proof-demo.sh` verified on Docker Server `29.0.1` + `scripts/public-node-proof.sh` verifies local public-listen status and one authenticated `FED_RESOLVE` round trip | not yet | `proof-demo.test.mjs`, `docker-demo.test.mjs`, `public-node-proof.test.mjs`, `bash scripts/docker-proof-demo.sh` | public reachability / hosted demo |
...
v10.22 is closed. The next natural boundary should keep making the accountability layer easier to verify externally: strengthen the public-node proof beyond one local `FED_RESOLVE`, continue Swarm proof only where it adds verifiable accountability, or add npm-facing packaging only if the existing Node exports are not enough.
