...
状态：v12.35 active
当前代码基线：`v12.35-docker-external-reachability-observer`
...
v12.35 adds `scripts/docker-external-reachability-observer.sh`, a Docker wrapper for the v12.34 TCP observer. It runs `scripts/external-reachability-observer.mjs` from a container, mounts the repo at `/app`, uses Docker host-gateway access for host-side proof targets, and keeps the observer trusted-Zone file outside the bundle. This supplies a stronger local runtime boundary for the v12.33 verifier gate, but still does not implement real outside-host reachability, hosted deployment, NAT traversal, package signing, scheduler ownership, batch verification, or A2A/ARD compatibility.
