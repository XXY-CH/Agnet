...
| Artifact write | done | deterministic local artifact + Go artifact manifest digest evidence + tool output digest alignment + local manifest sidecar/API + content-addressed local path | `mvp-demo.test.mjs`, `go-fed-discovery.test.mjs` | remote artifact store / GC |
...
v8.20 writes local artifacts under a content-addressed `artifacts/by-sha256/<digest>` path. The next natural boundary is remote artifact storage, container sandbox hardening, full transcript storage, or another small Ultimate-aligned runtime/governance slice.
