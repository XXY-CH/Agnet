...
| Reusable verifier | Node exports verifier functions + local npm-facing `asp-verify` bin contract, including stable receipt digest output, origin-Zone trust validation, artifact URI/SHA-256/manifest-hash output, proof summary receipt digest/count/URI/SHA-256/manifest-hash forwarding, narrow `FED_SWARM_CLOSE` signature/digest verification with structural step receipt checks, and trusted-Zone descriptor validation in CLI paths | `agnet/verifier.VerifyFederatedReceipt` package function for one `FED_RECEIPT` frame, including origin-Zone trust validation | `test-vectors.test.mjs`, `package-contract.test.mjs`, `proof-demo.test.mjs`, `public-node-proof.test.mjs`, `verifier/receipt_test.go`, `cmd/go-fed-discovery/main_test.go` | package publish/signing / Go module split / batch verifier |
...
Node
  ...
  -> asp-verify.mjs fed-receipt <frame.json> <trusted-zones.json> CLI with receipt origin-Zone trust validation
  -> asp-verify.mjs fed-receipt-artifacts <frame.json> <trusted-zones.json> CLI with receipt origin-Zone trust validation
  -> asp-verify.mjs swarm-close <frame.json> <trusted-zones.json> CLI for one FED_SWARM_CLOSE signature, digest, and structural step receipt check
...
v11.2 tightens Node Swarm close verification: a trusted Zone signature is no longer enough when `step_receipts` is empty or structurally malformed. The next natural boundary should keep moving toward Ultimate without widening the claim surface: add real public reachability proof only with external network evidence, or continue Swarm proof work only where it adds verifiable accountability without scheduler breadth or a Node audit verifier.
