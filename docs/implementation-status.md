...
| `FED_TASK_OPEN` | execute + Node client to Go task interop | execute minimal path + durable running/completed/failed task state files | `federation-gateway.mjs`, `go-fed-discovery.test.mjs` | scheduler / real restore |
...
v9.21 proves Node client to Go `FED_TASK_OPEN` interop and Go receipt verification. The next natural boundary should avoid further sandbox probe polish and move toward real container namespace execution proof, Go client to Node task interop, public transport/QUIC, or another small Ultimate-aligned runtime/governance slice.
