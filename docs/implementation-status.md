...
状态：v10.30 complete
当前代码基线：`v10.30-docker-public-listen-proof`
...
v10.30 is closed. The next natural boundary should keep making the accountability layer easier to verify externally: continue Swarm proof only where it adds verifiable accountability, add npm-facing packaging only if the existing Node exports are not enough, or add public reachability proof only after the Docker proof contract stays stable.
