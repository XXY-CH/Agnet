...
Go
  ...
  -> local public-listen proof for authenticated resolve, query, task execution, audit receipt query, artifact byte fetch, verifier-ready receipt/trust files, and fetched artifact byte verification
...
v10.27 is closed. The next natural boundary should keep making the accountability layer easier to verify externally: add negative coverage for `FED_ARTIFACT_READ` tampered or out-of-receipt artifact requests, continue Swarm proof only where it adds verifiable accountability, or add npm-facing packaging only if the existing Node exports are not enough.
