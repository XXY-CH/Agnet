...
Go
  ...
  -> verify-audit checks Swarm input_artifacts against declared after steps, upstream step artifact manifests, and receipt digest in the same audit
...
v9.29 binds Swarm dependency input step ids to the declared `after` DAG edges. The next natural boundary should either deepen Swarm orchestration without building a scheduler too early, align Node artifact manifests with Go, or start a real container namespace sandbox proof.
