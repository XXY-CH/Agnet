...
状态：v10.19 complete
当前代码基线：`v10.19-afp-artifact-manifests`
...
v10.19 is closed. The next natural boundary should keep making the accountability layer easier to verify externally: run and publish Docker proof output now that a Docker daemon is available, prepare a public-node proof path, continue Swarm proof only where it adds verifiable accountability, or add npm-facing packaging only if the existing Node exports are not enough.
