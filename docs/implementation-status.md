...
v10.23 is closed. The next natural boundary should keep making the accountability layer easier to verify externally: strengthen the public-node proof beyond local resolve/query, continue Swarm proof only where it adds verifiable accountability, or add npm-facing packaging only if the existing Node exports are not enough.
