...
v11.0 starts v11 by tightening receipt provenance: Node and Go `FED_RECEIPT` verifiers now reject signed receipts whose `origin_zone` is not in the trusted Zone store. The next natural boundary should keep moving toward Ultimate without widening the claim surface: add requester-origin binding only if the existing frame carries enough requester evidence, add real public reachability proof only with external network evidence, or continue Swarm proof work only where it adds verifiable accountability without scheduler breadth.
