...
| Product surface | CLI/tests only | thin Human Gateway with task table, queue table, and explicit local claim/drain actions | README, `go-fed-discovery.test.mjs` | approvals, task creation, admin, deployment |
...
Go
  ...
  -> Human Gateway /api/queue and explicit queue claim/drain actions
...
```text
v7.6 Queue Task Creation Boundary
  -> create signed queued tasks through an explicit local Human Gateway action
```
