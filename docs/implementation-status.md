...
| Artifact write | done | deterministic local artifact + Go artifact manifest digest evidence + tool output/transcript digest alignment + local manifest sidecar/API + content-addressed local path + optional filesystem artifact mirror + mirror object index + audit verifier byte/named sidecar/digest sidecar/mirror/index checks | `mvp-demo.test.mjs`, `go-fed-discovery.test.mjs` | object-store backend / GC |
...
v9.0 verifies the content-addressed object index for filesystem artifact mirrors. The next natural boundary is artifact GC over the verified index, container namespace sandboxing, object-store-backed artifacts, or another small Ultimate-aligned runtime/governance slice.
