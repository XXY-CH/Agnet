...
| Reproducible demo | `scripts/proof-demo.sh` local proof demo emits verifier-ready receipt/trust files and verifies local artifact closure + `scripts/docker-proof-demo.sh` verified on Docker Server `29.0.1` | not yet | `proof-demo.test.mjs`, `docker-demo.test.mjs`, `bash scripts/docker-proof-demo.sh` | public node / hosted demo |
...
v10.20 is closed. The next natural boundary should keep making the accountability layer easier to verify externally: prepare a public-node proof path, continue Swarm proof only where it adds verifiable accountability, or add npm-facing packaging only if the existing Node exports are not enough.
