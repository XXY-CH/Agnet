...
v12.34 adds `scripts/external-reachability-observer.mjs`, a minimal TCP observer that reads a proof bundle target, connects to its signed `transport_proof.listen_host:port`, and writes an observed bundle plus observer trusted-Zone file. This supplies runnable evidence for the v12.33 verifier gate, but still does not implement hosted deployment, NAT traversal, package signing, scheduler ownership, batch verification, or A2A/ARD compatibility.
