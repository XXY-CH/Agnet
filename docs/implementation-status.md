...
Go
  ...
  -> Human Gateway page browser-held requester key generation and signed draft submission
...
v8.6 adds a browser-held requester key panel that signs draft tasks into the existing queue draft path. The next natural boundary is key rotation/import-export UI or the next deployable transport/security hardening slice.

Route detail: `docs/v8-roadmap.md`。

Skipped until later: encrypted key store, key rotation/import-export UI, public transport, TLS/QUIC, login/session identity, token rotation/storage, container sandbox, Git/worktree/merge operations, scheduler queues, semantic routing.
