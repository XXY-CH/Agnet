...
Node
  ...
  -> asp-verify.mjs swarm-close <frame.json> <trusted-zones.json> CLI for one FED_SWARM_CLOSE signature and digest
...
v11.1 tightens task-entry provenance: `FED_TASK_OPEN` now requires the origin Zone to sign a requester Zone binding. The next natural boundary should keep moving toward Ultimate without widening the claim surface: add real public reachability proof only with external network evidence, or continue Swarm proof work only where it adds verifiable accountability without scheduler breadth.
