...
Node
  ...
  -> local artifact verifier rejects non-local artifact URIs before filesystem reads
...
v11.43 makes Node local artifact verification reject missing or non-local artifact URIs before filesystem reads. This keeps local artifact closure tied to the implemented artifact://local/ URI space without changing artifact manifest metadata, remote artifact fetch, object-store backends, retention policy, or Go verifier behavior. The next natural boundary should keep moving toward Ultimate without widening the claim surface: close remaining Phase A trust-boundary bugs only when still present in live code, add real public reachability proof only with external network evidence, or continue Swarm proof work only where it adds verifiable accountability without scheduler breadth.
