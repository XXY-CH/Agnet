...
Go
  ...
  -> Human Gateway page browser-held requester key generation, import/export, and signed draft submission
...
v8.7 adds explicit browser requester key import/export around the existing signed draft path. The next natural boundary is key rotation ceremony or the next deployable transport/security hardening slice.

Route detail: `docs/v8-roadmap.md`。

Skipped until later: encrypted key store, passphrase-protected export, key rotation ceremony, public transport, TLS/QUIC, login/session identity, token rotation/storage, container sandbox, Git/worktree/merge operations, scheduler queues, semantic routing.
