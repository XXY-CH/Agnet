...
v10.18 is closed. The next natural boundary should keep making the accountability layer easier to verify externally: run and publish Docker proof output once a Docker daemon is available, add AFP hash interoperability where it clarifies artifact proof, prepare a public-node proof path, continue Swarm proof only where it adds verifiable accountability, or add npm-facing packaging only if the existing Node exports are not enough.
