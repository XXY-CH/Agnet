...
Go
  ...
  -> local public-listen proof for authenticated resolve, query, task execution, audit receipt query, and verifier-ready receipt/trust files
...
v10.26 is closed. The next natural boundary should keep making the accountability layer easier to verify externally: add public-proof artifact-byte closure only after there is a truthful local fetch or mirror path for returned artifact refs, continue Swarm proof only where it adds verifiable accountability, or add npm-facing packaging only if the existing Node exports are not enough.
