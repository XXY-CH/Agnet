...
| Transport | local TCP / local process + authenticated session handshake | local TCP + optional TLS federation listener + minimal WebSocket + authenticated session handshake | README commands, `cmd/go-fed-discovery/main_test.go`, `federation-gateway.test.mjs`, `go-fed-discovery.test.mjs` | mTLS, QUIC, public bind |
...
v9.18 adds optional TLS on the main Go federation TCP listener. The next natural boundary should avoid further sandbox probe polish and move toward real container namespace execution proof, Go/Node interop, mTLS client certificate verification, or another small Ultimate-aligned runtime/governance slice.
