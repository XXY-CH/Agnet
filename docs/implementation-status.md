...
状态：v8.29 complete
当前代码基线：`v8.29-sandbox-env-binding`
...
v8.29 binds external/MCP tool HOME, TMPDIR, and XDG_CACHE_HOME to the local sandbox directory while preserving the honest local-process isolation label. The next natural boundary is container namespace sandboxing, login-backed sessions, streamed transcript UI, object-store-backed artifacts, or another small Ultimate-aligned runtime/governance slice.
