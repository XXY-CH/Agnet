...
状态：v12.38 active
当前代码基线：`v12.38-package-proof-asp-signature`
...
Node
  ...
  -> asp-verify.mjs package-proof <manifest.json> verifies package proof_digest, ASP package proof signature, manifest-relative tarball sha256, npm shasum, npm integrity, filename/tarball basename binding, manifest/input filename binding, package identity filename binding, packaged file list shape, and manifest-relative tarball size, returns verified package metadata including signer_aid, rejects non-object package proof manifests, and rejects unsafe package proof tarball paths
  ...
  -> scripts/docker-external-reachability-observer.sh Docker external reachability observer wrapper with host-gateway access and AGNET_NODE_BASE_IMAGE override
...
v12.38 adds ASP signing to the local package proof manifest. `scripts/package-proof.mjs` creates or reuses `state/keys/package-proof-signer.pkcs8`, writes a signer Agent descriptor and package proof signature into `package-proof.json`, and `asp-verify.mjs package-proof <manifest.json>` verifies that signer descriptor and signature before returning `signer_aid`. This is ASP-signed local package proof metadata, not npm registry signing, external signer trust pinning, SBOM, package publish, hosted reachability, scheduler ownership, batch verification, or A2A/ARD compatibility.
