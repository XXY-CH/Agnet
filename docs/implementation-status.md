...
状态：v9.16 complete
当前代码基线：`v9.16-container-runtime-binary-digest-probe`
...
v9.16 adds `runtime_sha256` binary digest evidence for readable `AGNET_CONTAINER_RUNTIME` candidates while still failing closed. The next natural boundary is real container namespace sandbox execution proof or another small Ultimate-aligned runtime/governance slice.
