...
v8.4 adds an optional bearer-token gate for Human Gateway writes. The next natural boundary is deployment transport/security hardening or browser key management UI.

Route detail: `docs/v8-roadmap.md`。

Skipped until later: encrypted key store, browser key management UI, public transport, TLS/QUIC, login/session identity, token rotation/storage, container sandbox, Git/worktree/merge operations, scheduler queues, semantic routing.
