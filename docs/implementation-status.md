...
| `FED_SWARM_OPEN` | not yet | explicit ordered two-step DAG seed; each step reuses signed task execution; receipts bind dependency artifact manifests; audit verifier rejects false signed dependency manifests | `go-fed-discovery.test.mjs`, `cmd/go-fed-discovery/main_test.go` | dynamic decomposition / scheduler / parallel execution / conflict resolution / cross-Zone Swarm |
...
Go
  ...
  -> verify-audit checks Swarm input_artifacts against upstream step artifact manifests in the same audit
...
v9.27 makes the Swarm seed auditable: `--verify-audit` now rejects a signed downstream Swarm receipt whose declared input artifact does not match the upstream step receipt in the same audit. The next natural boundary should either deepen Swarm orchestration without building a scheduler too early, align Node artifact manifests with Go, or start a real container namespace sandbox proof.
