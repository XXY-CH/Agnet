...
| `FED_SWARM_OPEN` | not yet | explicit ordered two-step DAG seed; each step reuses signed task execution and receipts bind dependency artifact manifests | `go-fed-discovery.test.mjs` | dynamic decomposition / scheduler / parallel execution / conflict resolution / cross-Zone Swarm |
...
v9.26 adds the first explicit Swarm seed: two Go workers execute a caller-supplied DAG and the downstream receipt signs the upstream artifact dependency. The next natural boundary should either deepen Swarm orchestration without building a scheduler too early, align Node artifact manifests with Go, or start a real container namespace sandbox proof.
