...
状态：v11.69 active
当前代码基线：`v11.69-go-swarm-open-after-list-shape-boundary`
...
v11.69 makes Go `FED_SWARM_OPEN` reject malformed step `after` list entries before executing dependent steps. This keeps the Swarm open boundary narrow: it does not add dynamic decomposition, scheduler ownership, parallel execution, cross-Zone Swarms, or generic Swarm schema validation. The next natural boundary should keep moving toward Ultimate without widening the claim surface: close remaining Phase A trust-boundary bugs only when still present in live code, add real public reachability proof only with external network evidence, or continue Swarm proof work only where it adds verifiable accountability without scheduler breadth.
