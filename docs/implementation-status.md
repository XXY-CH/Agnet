...
v11.65 makes Go receipt approval verification reject malformed `approvals` and `approval_grants` list entries before approval grant count and signature checks. This keeps the approval evidence boundary narrow: it does not add a role system, change approval policy semantics, or alter Human Gateway session behavior. The next natural boundary should keep moving toward Ultimate without widening the claim surface: close remaining Phase A trust-boundary bugs only when still present in live code, add real public reachability proof only with external network evidence, or continue Swarm proof work only where it adds verifiable accountability without scheduler breadth.
