...
| Transport | local TCP / local process + authenticated session handshake | local TCP + optional TLS/mTLS federation listener + minimal WebSocket + authenticated session handshake | README commands, `cmd/go-fed-discovery/main_test.go`, `federation-gateway.test.mjs`, `go-fed-discovery.test.mjs` | certificate-to-Zone binding, QUIC, public bind |
...
v9.19 adds optional mTLS client certificate verification on the main Go federation TCP listener. The next natural boundary should avoid further sandbox probe polish and move toward real container namespace execution proof, Go/Node interop, certificate-to-Zone binding, or another small Ultimate-aligned runtime/governance slice.
