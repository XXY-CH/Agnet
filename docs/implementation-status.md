...
状态：v13 active at v13.12
当前代码基线：`v13.12-credential-validity-window`
...
The active protocol tag is `v13.12-protocol`: credential validity window is the latest complete slice, while the v13.1 hosted external-host observer run and hardware remote attestation remain pending exit criteria.
...
| Capability query | done + Node federation gateway evidence-first semantic discovery/reputation ranking with persisted audit-backed receipt counts | done + Go FED_QUERY semantic intent scoring, evidence-first ranking, discovery_evidence, ranking.score/reasons, and persisted audit-backed receipt counts | `federation-gateway.test.mjs`, `go-fed-discovery.test.mjs` | scheduling |
...
v13 is active at `v13.12-protocol`, with v13.12 credential validity window as the latest complete slice. Capability credentials may carry a `valid_until` ISO UTC expiry in claims; expired credentials lower discovery score and report `active: false` in discovery evidence. v13.11 audit-backed receipt-count reputation remains complete: receipt counts come from the persisted audit log; this is not a hardcoded demo value, not cross-session ML, not a global reputation oracle. v13.1 reachability evidence gates remain active: verifier-owned scope classes and observer evidence binding landed with tests; real hosted external-host evidence is still pending as the remaining v13.1 exit criterion. The current hosted attempt is recorded as GitHub Actions run `28916288568`, which failed with `ENETUNREACH` from the hosted runner to the current IPv6 listener.
