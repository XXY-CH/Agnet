...
v12.32 makes Go protocol signing, signature verification, and digest paths use canonical JSON without HTML escaping for `<`, `>`, and `&`, matching Node canonical bytes for signed and digested protocol objects. This keeps the boundary narrow: it does not implement external public reachability, package signing, SBOM, scheduler ownership, batch verification, or A2A/ARD compatibility.
