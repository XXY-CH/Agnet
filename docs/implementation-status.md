...
| Artifact write | done | deterministic local artifact + Go artifact manifest digest evidence + tool output digest alignment + local manifest sidecar/API + content-addressed local path + audit verifier byte checks | `mvp-demo.test.mjs`, `go-fed-discovery.test.mjs` | remote artifact store / GC |
...
v8.21 makes `--verify-audit` check local artifact bytes against signed artifact manifests. The next natural boundary is remote artifact storage, container sandbox hardening, full transcript storage, or another small Ultimate-aligned runtime/governance slice.
