...
| Reproducible demo | `scripts/proof-demo.sh` local proof demo | not yet | `proof-demo.test.mjs` | Docker / public node / hosted demo |
...
v10.9 is closed. The next natural boundary should keep making the accountability layer easier to verify externally: provide a reproducible Docker demo, or add npm-facing packaging only if the existing Node exports are not enough.
