...
v8.8 adds a browser-side requester key rotation proof ceremony around the existing signed draft path. The next natural boundary is Zone alias rebinding UI/API or the next deployable transport/security hardening slice.

Route detail: `docs/v8-roadmap.md`。

Skipped until later: Zone alias rebinding, server-side rotation registry, encrypted key store, passphrase-protected export, public transport, TLS/QUIC, login/session identity, token rotation/storage, container sandbox, Git/worktree/merge operations, scheduler queues, semantic routing.
