...
v11.31 makes Node Zone descriptor verification reject missing, array, or otherwise non-object descriptors before reading descriptor fields. This keeps trusted-Zone loading and direct descriptor verification from leaking low-level TypeErrors without adding generic schema validation, per-caller guards, Go verifier changes, scheduler routing, or dynamic Swarm decomposition. The next natural boundary should keep moving toward Ultimate without widening the claim surface: close remaining Phase A trust-boundary bugs only when still present in live code, add real public reachability proof only with external network evidence, or continue Swarm proof work only where it adds verifiable accountability without scheduler breadth.
