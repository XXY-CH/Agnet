...
| Upper-layer demo | static master-agent signed 10-agent delegation plan | not yet | `upper-layer-demo.mjs`, `upper-layer-demo.test.mjs` | assignment execution / scheduler / dynamic routing |
...
Node
  ...
  -> asp-verify.mjs package-proof <manifest.json> verifies package proof_digest, manifest-relative tarball sha256, npm shasum, npm integrity, filename/tarball basename binding, manifest/input filename binding, package identity filename binding, packaged file list shape, and manifest-relative tarball size, returns verified package metadata, rejects non-object package proof manifests, and rejects unsafe package proof tarball paths
...
v12.36 adds `upper-layer-demo.mjs`, a static signed demo skeleton for the final-stage upper layer: one master agent, ten specialist agents, a Zone-signed registry, and a master-agent-signed delegation plan. This supplies a verifiable top-layer entry point over the existing identity/registry/signature substrate, but still does not execute assignments, implement scheduler ownership, dynamic routing, hosted reachability, package signing, batch verification, or A2A/ARD compatibility.
