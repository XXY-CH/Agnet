...
| `FED_TASK_ENQUEUE` | not yet | durable local queue file + claim/lease + explicit drain path | `go-fed-discovery.test.mjs` | lease expiry / reclaim / retry / automatic drain |
...
Go
  ...
  -> FED_QUEUE_CLAIM lease ownership
...
```text
v7.3 Queue Lease Reclaim Boundary
  -> expire or reclaim claimed work before retrying ownership
```
