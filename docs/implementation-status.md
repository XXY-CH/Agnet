...
| Local task execution | done | built-in + external stdio + MCP stdio tools/call + local temp-dir sandbox evidence + Git worktree context evidence | `agent-runtime.test.mjs`, `go-fed-discovery.test.mjs` | container sandbox / long-running MCP sessions |
...
| Scoped workspace | prototype | task-scoped detached Git worktree for Go tools | `go-fed-discovery.test.mjs` | signed merge / conflict resolution / realtime context sync |
...
```text
Signed merge + checkpoint receipts
  -> worktree output promotion and resumable long tasks
```

Skipped until later: encrypted key store, public transport, interactive approval queue, container sandbox, realtime context sync, A2A translation layer, scheduling, semantic routing.
