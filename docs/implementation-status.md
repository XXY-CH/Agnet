...
| Local task execution | done | built-in + external stdio + MCP stdio tools/call + MCP initialize/resources/prompts metadata + signed local sandbox proof + sandbox claim binding + tool command digest | `agent-runtime.test.mjs`, `go-fed-discovery.test.mjs` | container sandbox / long-running MCP sessions |
...
v5.9 MCP tool list evidence
  -> bind available tool surface before tools/call
```
