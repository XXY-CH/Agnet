...
| Audit hash chain | done | done for Go execution, queue actions, remote receipt proof query, and Human Gateway task-scoped receipt proof query | `audit-chain.test.mjs`, `go-fed-discovery.test.mjs` | full log sync / remote search |
...
Go
  ...
  -> Human Gateway receipt table links artifact refs to /api/artifacts/manifest
...
v8.41 exposes signed receipt digest headers on receipt-scoped artifact reads. The next natural boundary is container namespace sandboxing, streamed transcript UI, object-store-backed artifacts, or another small Ultimate-aligned runtime/governance slice.
