...
| Artifact write | done | deterministic local artifact + Go artifact manifest digest evidence + tool output digest alignment + local manifest sidecar/API + content-addressed local path + audit verifier byte/sidecar checks | `mvp-demo.test.mjs`, `go-fed-discovery.test.mjs` | remote artifact store / GC |
...
v8.22 makes `--verify-audit` check named artifact manifest sidecars against signed receipt manifests. The next natural boundary is digest-addressed sidecar verification, remote artifact storage, container sandbox hardening, full transcript storage, or another small Ultimate-aligned runtime/governance slice.
