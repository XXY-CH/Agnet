...
状态：v13 active at v13.1
当前代码基线：`v13.1-reachability-scope`
...
The active protocol tag remains `v13.0-protocol`: `local-interface`, `container-observer`, and `external-host` are verifier-owned reachability scopes, and `external-host` now requires a globally routable literal-IP listen host; hostname listen hosts are out of scope for this slice.
