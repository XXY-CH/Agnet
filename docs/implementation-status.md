...
v11.63 makes Go audit-backed Swarm dependency verification reject malformed `after` and `input_artifacts` list entries before dependency count and digest checks. This keeps the Swarm proof boundary narrow: it does not add dynamic decomposition, scheduler ownership, parallel execution, cross-Zone Swarms, or a Node audit verifier. The next natural boundary should keep moving toward Ultimate without widening the claim surface: close remaining Phase A trust-boundary bugs only when still present in live code, add real public reachability proof only with external network evidence, or continue Swarm proof work only where it adds verifiable accountability without scheduler breadth.
