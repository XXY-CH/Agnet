...
| Local task execution | done | built-in + external stdio + MCP stdio tools/call + MCP initialize metadata + signed local sandbox proof + sandbox claim binding + tool command digest | `agent-runtime.test.mjs`, `go-fed-discovery.test.mjs` | container sandbox / long-running MCP sessions |
...
v5.8 MCP resources/prompts evidence
  -> query richer MCP metadata without long-running session reuse
```
