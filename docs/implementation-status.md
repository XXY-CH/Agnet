...
v12.33 lets `asp-verify.mjs proof-bundle <bundle.json> [external-trusted-zones.json]` verify optional signed external reachability evidence from a caller-supplied trusted observer Zone. The default proof remains `reachability_scope: "local-interface"`; `external-host` appears only when the observer signature binds the same transport proof and receipt digest. This still does not implement hosted deployment, NAT traversal, package signing, scheduler ownership, batch verification, or A2A/ARD compatibility.
