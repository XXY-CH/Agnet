...
v10.17 is closed. The next natural boundary should keep making the accountability layer easier to verify externally: reject delimiter-breaking Swarm ids, run and publish Docker proof output once a Docker daemon is available, continue Swarm proof only where it adds verifiable accountability, or add npm-facing packaging only if the existing Node exports are not enough.
