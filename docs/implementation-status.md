...
v8.9 adds a Zone-signed requester alias rebinding proof endpoint after browser requester key rotation. The next natural boundary is persistent alias binding registry update, browser rebinding UI, or the next deployable transport/security hardening slice.

Route detail: `docs/v8-roadmap.md`。

Skipped until later: persistent registry alias updates, browser rebinding UI, server-side rotation registry, encrypted key store, passphrase-protected export, public transport, TLS/QUIC, login/session identity, token rotation/storage, container sandbox, Git/worktree/merge operations, scheduler queues, semantic routing.
