...
状态：v13 active at v13.11
当前代码基线：`v13.11-audit-backed-receipt-count-reputation`
...
The active protocol tag is `v13.11-protocol`: audit-backed receipt-count reputation is the latest complete slice, while the v13.1 hosted external-host observer run and hardware remote attestation remain pending exit criteria.
...
v13 is active at `v13.11-protocol`, with v13.11 audit-backed receipt-count reputation as the latest complete slice. receipt counts come from the persisted audit log; this is not a hardcoded demo value, not cross-session ML, not a global reputation oracle. v13.1 reachability evidence gates remain active: verifier-owned scope classes and observer evidence binding landed with tests; real hosted external-host evidence is still pending as the remaining v13.1 exit criterion. The current hosted attempt is recorded as GitHub Actions run `28916288568`, which failed with `ENETUNREACH` from the hosted runner to the current IPv6 listener.
