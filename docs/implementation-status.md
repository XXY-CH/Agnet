...
| Reproducible demo | `scripts/proof-demo.sh` local proof demo emits verifier-ready receipt/trust files and verifies local artifact closure + `scripts/docker-proof-demo.sh` verified on Docker Server `29.0.1` and accepts `AGNET_NODE_BASE_IMAGE` + `scripts/public-node-proof.sh` verifies local public-listen status and authenticated `FED_RESOLVE` / `FED_QUERY` / `FED_TASK_OPEN` / `FED_AUDIT_QUERY` / `FED_ARTIFACT_READ` / `FED_SWARM_OPEN` round trips, emits verifier-ready receipt/trust files and `state/public-node-proof-swarm-close.json`, verifies fetched artifact bytes, proves out-of-receipt plus post-receipt-tampered artifact reads are rejected, and verifies a two-step Swarm close proof with `swarm_close_digest` + `scripts/docker-public-node-proof.sh` verifies the same public-listen proof inside Docker and accepts `AGNET_GO_BASE_IMAGE` / `AGNET_NODE_BASE_IMAGE` | not yet | `proof-demo.test.mjs`, `docker-demo.test.mjs`, `public-node-proof.test.mjs`, `bash scripts/docker-proof-demo.sh`, `bash scripts/docker-public-node-proof.sh` | public reachability / hosted demo |
