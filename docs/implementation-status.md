...
Go
  ...
  -> filesystem artifact mirrors maintain and verify an objects.ndjson content-addressed object index with required SHA-256 path validation, present manifest-hash digest validation, object-entry validation, and exact manifest field matching
  ...
  -> audit verifier checks optional filesystem artifact mirror bytes, sidecars, required safe SHA-256 index paths, present manifest-hash index digests, object-shaped index entries, and exact-type object index matches when --artifact-store is configured
