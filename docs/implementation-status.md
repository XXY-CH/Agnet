...
v8.32 exposes read-only local approval session state through `/api/session`. The next natural boundary is login-state UI, container namespace sandboxing, streamed transcript UI, object-store-backed artifacts, or another small Ultimate-aligned runtime/governance slice.
