...
| Human approval | simulated | direct Go task execution and queued drain write pending approval state; approve continues, deny/expiry stops before tools; direct approvals preserve named `human://...` actors in signed grants | Node events, `go-fed-discovery.test.mjs` | login-state UI / roles |
...
v8.25 preserves named local human actors in signed direct approval evidence. The next natural boundary is approval actor policy, remote artifact storage, container sandbox hardening, login-backed sessions, streamed transcript UI, or another small Ultimate-aligned runtime/governance slice.
