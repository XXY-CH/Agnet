...
test("v11 public docs include FED_TASK_OPEN worker descriptor identity", async () => {
  ...
  assert.match(status, /状态：v12\.36 active/);
...
test("v11 public docs include object signature fail-closed verification", async () => {
  ...
  assert.match(readme, /v12 active at `v12\.36-protocol`/);
...
test("v12 public docs start with public proof bundle manifests", async () => {
  ...
  assert.match(status, /状态：v12\.36 active/);
  assert.match(status, /v12.36-upper-layer-demo-plan/);
...
test("v12 public docs include proof bundle path safety", async () => {
  ...
  assert.match(draft, /v12\.36-protocol/);
  assert.match(status, /状态：v12\.36 active/);
  assert.match(status, /v12.36-upper-layer-demo-plan/);
...
test("v12 public docs include upper-layer demo plan", async () => {
  const [readme, roadmap, draft, status, boundary] = await Promise.all([
    readFile("README.md", "utf8"),
    readFile("docs/v12-roadmap.md", "utf8"),
    readFile("docs/asp-core-draft.md", "utf8"),
    readFile("docs/implementation-status.md", "utf8"),
    readFile("docs/v12.36-boundary.md", "utf8"),
  ]);

  assert.match(readme, /v12 active at `v12\.36-protocol`/);
  assert.match(readme, /`docs\/v12\.36-boundary\.md` - latest closed boundary\./);
  assert.match(readme, /upper-layer-demo\.mjs/);
  assert.match(readme, /one master agent, ten specialists/);
  assert.match(roadmap, /## v12\.36: Upper-Layer Demo Plan/);
  assert.match(roadmap, /master-agent-signed static delegation plan/);
  assert.match(draft, /upper-layer-demo\.mjs/);
  assert.match(draft, /one master agent, ten specialist agents/);
  assert.match(status, /状态：v12\.36 active/);
  assert.match(status, /v12.36-upper-layer-demo-plan/);
  assert.match(status, /upper-layer master-agent signed 10-agent demo plan/);
  assert.match(boundary, /10 specialist agents and one master agent/);
  assert.match(boundary, /not an autonomous orchestration runtime/);
});
