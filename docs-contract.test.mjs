...
test("v11 public docs include FED_TASK_OPEN worker descriptor identity", async () => {
  ...
  assert.match(status, /状态：v11\.76 active/);
...
test("v11 public docs include object signature fail-closed verification", async () => {
  ...
  assert.match(readme, /v11 active at `v11\.76-protocol`/);
...
test("v11 public docs include Go receipt task id token validation", async () => {
  ...
  assert.match(readme, /v11 active at `v11\.76-protocol`/);
  assert.match(readme, /`docs\/v11\.76-boundary\.md` - latest closed boundary\./);
  assert.match(readme, /Go receipt verification rejects unsafe receipt task ids/);
  assert.match(roadmap, /## v11\.76: Go Receipt Task ID Token Boundary/);
  assert.match(draft, /receipt verification rejects unsafe signed receipt `task_id` values/);
  assert.match(status, /状态：v11\.76 active/);
  assert.match(status, /FED_RECEIPT verifier rejects unsafe Go receipt task ids/);
