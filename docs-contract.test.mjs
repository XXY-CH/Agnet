...
test("v11 public docs include FED_TASK_OPEN worker descriptor identity", async () => {
  ...
  assert.match(status, /状态：v12\.12 active/);
...
test("v11 public docs include object signature fail-closed verification", async () => {
  ...
  assert.match(readme, /v12 active at `v12\.12-protocol`/);
...
test("v12 public docs start with public proof bundle manifests", async () => {
  ...
  assert.match(status, /状态：v12\.12 active/);
  assert.match(status, /v12\.12-proof-bundle-transport-shape/);
...
test("v12 public docs include proof bundle path safety", async () => {
  ...
  assert.match(draft, /v12\.12-protocol/);
  assert.match(status, /状态：v12\.12 active/);
  assert.match(status, /v12\.12-proof-bundle-transport-shape/);
...
test("v12 public docs include proof bundle transport proof shape", async () => {
  ...
  assert.match(readme, /v12 active at `v12\.12-protocol`/);
  assert.match(readme, /`docs\/v12\.12-boundary\.md` - latest closed boundary\./);
  assert.match(readme, /required signed `transport` \/ `listen_host` \/ `port` \/ `public_transport: true` proof/);
  assert.match(roadmap, /## v12\.12: Proof Bundle Transport Proof Shape/);
  assert.match(draft, /non-empty `transport`, non-empty `listen_host`, decimal-string `port`, and `public_transport: true`/);
  assert.match(status, /状态：v12\.12 active/);
  assert.match(status, /v12\.12-proof-bundle-transport-shape/);
