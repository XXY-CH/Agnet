...
test("v11 public docs include FED_TASK_OPEN worker descriptor identity", async () => {
  ...
  assert.match(status, /状态：v11\.53 active/);
...
test("v11 public docs include object signature fail-closed verification", async () => {
  ...
  assert.match(readme, /v11 active at `v11\.53-protocol`/);
...
test("v11 public docs include Go artifact mirror index digest validation", async () => {
  ...
  assert.match(readme, /v11 active at `v11\.53-protocol`/);
  assert.match(readme, /`docs\/v11\.53-boundary\.md` - latest closed boundary\./);
  ...
  assert.match(draft, /MUST reject index entries whose `sha256` field is present but not a 64-hex digest/);
  assert.match(status, /状态：v11\.53 active/);
