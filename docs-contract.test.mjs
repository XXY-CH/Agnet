...
test("v11 public docs include FED_TASK_OPEN worker descriptor identity", async () => {
  ...
  assert.match(status, /状态：v11\.71 active/);
...
test("v11 public docs include object signature fail-closed verification", async () => {
  ...
  assert.match(readme, /v11 active at `v11\.71-protocol`/);
...
test("v11 public docs include Go task write scope list shape validation", async () => {
  ...
  assert.match(readme, /v11 active at `v11\.71-protocol`/);
  assert.match(readme, /`docs\/v11\.71-boundary\.md` - latest closed boundary\./);
  assert.match(readme, /malformed signed task write scopes fail closed/);
  ...
  assert.match(status, /状态：v11\.71 active/);
  assert.match(status, /Go signed task write-scope list shape validation/);
