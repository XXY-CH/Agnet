...
test("v11 public docs include FED_TASK_OPEN worker descriptor identity", async () => {
  ...
  assert.match(status, /状态：v12\.38 active/);
...
test("v11 public docs include object signature fail-closed verification", async () => {
  ...
  assert.match(readme, /v12 active at `v12\.38-protocol`/);
...
test("v12 public docs start with public proof bundle manifests", async () => {
  ...
  assert.match(status, /状态：v12\.38 active/);
  assert.match(status, /v12.38-package-proof-asp-signature/);
...
test("v12 public docs include proof bundle path safety", async () => {
  ...
  assert.match(draft, /v12\.38-protocol/);
  assert.match(status, /状态：v12\.38 active/);
  assert.match(status, /v12.38-package-proof-asp-signature/);
...
test("v12 public docs include package proof ASP signature", async () => {
  ...
  assert.match(readme, /v12 active at `v12\.38-protocol`/);
  assert.match(readme, /`docs\/v12\.38-boundary\.md` - latest closed boundary\./);
  assert.match(readme, /ASP package proof signature/);
  assert.match(readme, /`signer_aid` fields/);
  ...
  assert.match(draft, /not npm registry signing, external signer trust pinning, package publish, or SBOM/);
  assert.match(status, /状态：v12\.38 active/);
  assert.match(status, /v12.38-package-proof-asp-signature/);
