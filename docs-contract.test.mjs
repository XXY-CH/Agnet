...
test("v13 public docs open the Ultimate-facing protocol milestone without scope drift", async () => {
  ...
  assert.match(readme, /v13 active at `v13\.12-protocol`/);
  ...
  assert.match(roadmap, /状态：active at v13\.12/);
  ...
  assert.match(draft, /local-first prototype at `v13\.12-protocol`/);
  assert.match(status, /状态：v13 active at v13\.12/);
  assert.match(status, /v13.12-credential-validity-window/);
...
test("v13.2 public docs describe release trust without supply-chain overclaims", async () => {
  ...
  assert.match(status, /状态：v13 active at v13\.12/);
  assert.match(status, /当前代码基线：`v13\.12-credential-validity-window`/);
...
test("v13.12 public docs record credential valid_until expiry without credential lifecycle overclaims", async () => {
  ...
  for (const doc of [readme, roadmap, boundary, status, draft]) {
    ...
    assert.doesNotMatch(doc, /revocation lists/i);
