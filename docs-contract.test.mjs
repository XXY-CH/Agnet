...
test("v13 public docs open the Ultimate-facing protocol milestone without scope drift", async () => {
  ...
  assert.match(readme, /v13 active at `v13\.14-protocol`/);
  ...
  assert.match(roadmap, /状态：active at v13\.14/);
  ...
  assert.match(draft, /local-first prototype at `v13\.14-protocol`/);
  assert.match(status, /状态：v13 active at v13\.14/);
  assert.match(status, /v13.14-agent-score-reputation/);
...
test("v13.2 public docs describe release trust without supply-chain overclaims", async () => {
  ...
  assert.match(status, /状态：v13 active at v13\.14/);
  assert.match(status, /当前代码基线：`v13\.14-agent-score-reputation`/);
...
test("v13.4 public docs describe evidence-first semantic discovery without marketplace overclaims", async () => {
  ...
  assert.match(draft, /Ranking is deterministic and evidence-first: exact capability match, trusted capability credential, signed credential claims, audit-backed reputation, and semantic token overlap are exposed as inspectable evidence/);
  assert.match(status, /状态：v13 active at v13\.14/);
  assert.match(status, /当前代码基线：`v13\.14-agent-score-reputation`/);
...
test("v13.11 public docs record audit-backed receipt-count reputation without oracle overclaims", async () => {
  ...
  for (const doc of [readme, roadmap, boundary, status, draft]) {
    ...
    assert.match(doc, /not a hardcoded demo value, not cross-session learned scoring, and not a third-party reputation service/);
...
test("v13.13 public docs record zone revocation in discovery without oracle overclaims", async () => {
  ...
  for (const doc of [readme, roadmap, boundary, status, draft]) {
    ...
    assert.match(doc, /revoked workers lose credential contribution in `agent_score`/);
