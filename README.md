...
- Latest boundary: [docs/v7.13-boundary.md](docs/v7.13-boundary.md)
...
- Go accepts `FED_TASK_RESUME` only when the parent checkpoint exists in audit, then records resumed receipts linked to that parent checkpoint.
- Go accepts `FED_QUEUE_RESUME` only when the parent checkpoint exists in audit, then preserves the resume binding through queued drain.
...
- Human Gateway queue actions create `go_queue_action` audit evidence.
- Human Gateway queue actions require signed action grants with actor binding, local actor policy, action scope, expiry, and replay rejection.
- Go drains a queued task explicitly through `FED_QUEUE_DRAIN` and records completed queue state.
