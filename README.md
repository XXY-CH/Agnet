...
Status: research prototype, local-first, v10 active at `v10.26-protocol`.
...
- Public-listen proof script that starts the Go federation gateway on `0.0.0.0`, proves `public_transport: true`, completes authenticated `FED_RESOLVE`, `FED_QUERY`, `FED_TASK_OPEN`, and `FED_AUDIT_QUERY` round trips, and emits verifier-ready receipt/trust files.
