...
Status: research prototype, local-first, v11 active at `v11.21-protocol`.
...
- `FED_TASK_OPEN` verifies frame object/type and requester Zone bindings at the federation boundary.
...
- Node receipt verifiers reject missing/non-`FED_RECEIPT` frames, Go receipt verifiers reject non-`FED_RECEIPT` frames, and signed receipts whose `origin_zone` is not trusted fail closed.
...
- `docs/v11.21-boundary.md` - latest closed boundary.
