...
Status: research prototype, local-first, v10 active at `v10.27-protocol`.
...
- Public-listen proof script that starts the Go federation gateway on `0.0.0.0`, proves `public_transport: true`, completes authenticated `FED_RESOLVE`, `FED_QUERY`, `FED_TASK_OPEN`, `FED_AUDIT_QUERY`, and `FED_ARTIFACT_READ` round trips, and verifies fetched artifact bytes.
