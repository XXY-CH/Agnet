...
Status: research prototype, local-first, v11 active at `v11.22-protocol`.
...
- `FED_TASK_OPEN` verifies frame object/type, origin Zone descriptor presence, and requester Zone bindings at the federation boundary.
...
- Node receipt verifiers reject missing/non-`FED_RECEIPT` frames and missing signing Zone descriptors, Go receipt verifiers reject non-`FED_RECEIPT` frames, and signed receipts whose `origin_zone` is not trusted fail closed.
...
- `docs/v11.22-boundary.md` - latest closed boundary.
