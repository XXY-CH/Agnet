...
func parseWorkerMessage(raw []byte, target any) error {
	if len(raw) == 0 || len(raw) > localSwarmWorkerMaxMessageBytes || !json.Valid(raw) || validateSwarmJSONNoDuplicateFields(raw) != nil { return errors.New("local swarm worker message invalid") }
	decoder := json.NewDecoder(bytes.NewReader(raw))
	decoder.DisallowUnknownFields()
	if err := decoder.Decode(target); err != nil || ensureSwarmJSONEOF(decoder) != nil { return errors.New("local swarm worker message invalid") }
	canonical, err := canonicalWorkerMessage(target)
	if err != nil || !bytes.Equal(raw, canonical) { return errors.New("local swarm worker message noncanonical") }
	return nil
}

func ParseSwarmWorkerRequest(raw []byte) (SwarmWorkerRequest, error) {
	var request SwarmWorkerRequest
	if err := parseWorkerMessage(raw, &request); err != nil { return request, err }
	return request, request.validate()
}

func ParseSwarmWorkerResult(raw []byte) (SwarmWorkerResult, error) {
	var result SwarmWorkerResult
	if err := parseWorkerMessage(raw, &result); err != nil { return result, err }
	return result, result.validate()
}

type SwarmWorkerLauncher interface { Launch(context.Context, SwarmWorkerRequest) (SwarmWorkerResult, error) }
...
func defaultSwarmWorkerCommand(ctx context.Context) (*exec.Cmd, error) {
	...
	if err != nil { return nil, errors.New("local swarm worker executable unavailable") }
...
func (launcher ExecSwarmWorkerLauncher) Launch(ctx context.Context, request SwarmWorkerRequest) (SwarmWorkerResult, error) {
	...
	if err != nil { return SwarmWorkerResult{}, err }
	runner := launcher.runner
	if runner == nil { runner = defaultSwarmWorkerCommand }
	command, err := runner(ctx)
	if err != nil || command == nil { return SwarmWorkerResult{}, errors.New("local swarm worker executable unavailable") }
	command.Cancel = func() error {
		if command.Process == nil { return nil }
		return command.Process.Signal(syscall.SIGTERM)
	}
	command.WaitDelay = localSwarmWorkerWaitDelay
	stdin, err := command.StdinPipe(); if err != nil { return SwarmWorkerResult{}, errors.New("local swarm worker pipe unavailable") }
	stdout, err := command.StdoutPipe(); if err != nil { return SwarmWorkerResult{}, errors.New("local swarm worker pipe unavailable") }
	command.Stderr = io.Discard
	if err := command.Start(); err != nil { return SwarmWorkerResult{}, errors.New("local swarm worker start failed") }
	if _, err := stdin.Write(raw); err != nil { _ = stdin.Close(); _ = command.Wait(); return SwarmWorkerResult{}, errors.New("local swarm worker request failed") }
	if err := stdin.Close(); err != nil { _ = command.Wait(); return SwarmWorkerResult{}, errors.New("local swarm worker request failed") }
	resultRaw, readErr := io.ReadAll(io.LimitReader(stdout, localSwarmWorkerMaxMessageBytes+1))
	waitErr := command.Wait()
	if readErr != nil || len(resultRaw) > localSwarmWorkerMaxMessageBytes || waitErr != nil { return SwarmWorkerResult{}, errors.New("local swarm worker failed") }
	result, err := ParseSwarmWorkerResult(resultRaw)
	if err != nil { return SwarmWorkerResult{}, err }
	if err := verifyCommittedWorkerResult(request, result); err != nil { return SwarmWorkerResult{}, err }
	return result, nil
}

func verifyCommittedWorkerResult(request SwarmWorkerRequest, result SwarmWorkerResult) error {
	if result.SwarmID != request.SwarmID || result.StepID != request.StepID || result.Fence != request.Fence { return errors.New("local swarm worker result does not match request") }
	journal, err := OpenSwarmJournal(request.StorageRoot, request.SwarmID)
	if err != nil { return errors.New("local swarm worker journal unavailable") }
	entries, err := journal.Replay()
	if err != nil { return errors.New("local swarm worker journal unavailable") }
	if _, err := ReduceSwarmEntries(entries); err != nil { return errors.New("local swarm worker journal unavailable") }
	for _, entry := range entries {
		if entry.Kind != "receipt.committed" { continue }
		var payload receiptCommittedPayload
		if err := decodeStrictSwarmPayload(entry.Payload, &payload); err != nil { return errors.New("local swarm worker journal unavailable") }
		if payload.Claim.StepID != request.StepID || payload.Claim.Owner != request.Owner || payload.Claim.Fence != request.Fence || payload.ReceiptDigest != result.ReceiptDigest { continue }
		if err := payload.validateCanonical(); err != nil || !validArtifactTriple(payload.Result) { return errors.New("local swarm worker journal unavailable") }
		return nil
	}
...
func (deps LocalSwarmWorkerDeps) normalized() (LocalSwarmWorkerDeps, error) {
	...
	if deps.LeaseTTL <= 0 { deps.LeaseTTL = 30 * time.Second }
	if deps.Heartbeat <= 0 { deps.Heartbeat = deps.LeaseTTL / 3 }
	if deps.Heartbeat <= 0 || deps.Heartbeat >= deps.LeaseTTL/2 { return deps, errors.New("local swarm worker heartbeat must be below half ttl") }
	return deps, nil
}
	...
	go func() {
		...
		for {
			...
			case <-ticker.C:
				...
				if err != nil { errs <- errors.New("lease heartbeat renewal failed"); cancel(); return }
				live = renewed
			}
		}
	}()
...
func exactLiveClaim(journal *SwarmJournal, request SwarmWorkerRequest) (SwarmState, LeaseClaim, error) {
	entries, err := journal.Replay(); if err != nil { return SwarmState{}, LeaseClaim{}, err }
	state, err := ReduceSwarmEntries(entries); if err != nil { return SwarmState{}, LeaseClaim{}, err }
	for _, claim := range state.Leases {
		if claim.StepID == request.StepID && claim.Owner == request.Owner && claim.Fence == request.Fence { return state, claim, nil }
	}
...
func localWorkerProfile(candidate DurableWorkerCandidate) (WorkerProfile, error) {
	if candidate.Runtime == nil { return WorkerProfile{}, errors.New("local swarm worker runtime missing") }
	profile := *candidate.Runtime
	if profile.KeyFile != "" || profile.KeyStore != "" || profile.PassphraseFile != "" || profile.KeyGeneration != (WorkerGenerationPin{}) { return WorkerProfile{}, errors.New("local swarm worker runtime must not carry key material") }
	if profile.SandboxClaim != "container-namespace" || profile.Docker == nil { return WorkerProfile{}, errors.New("local swarm worker requires container runtime") }
	if _, err := validateDockerWorkerProfile(*profile.Docker); err != nil { return WorkerProfile{}, err }
	return profile, nil
}
...
func localWorkerAdapter(runtimeKind string) (DockerAdapter, error) {
	...
	case "apple-container":
		...
		if err != nil || !filepath.IsAbs(home) { return nil, errors.New("local swarm worker Apple runtime unavailable") }
	...
	case "docker":
		host := DockerHost{CommandPath: dockerCommandPath, SocketPath: dockerLocalUnixSocket, Environment: dockerSanitizedEnvironment,
			BinaryDigest: func(path string) (string, error) { data, err := os.ReadFile(path); if err != nil { return "", err }; hash := sha256.Sum256(data); return fmt.Sprintf("%x", hash[:]), nil },
			SocketIdentity: func(path string) (DockerSocketIdentity, error) { info, err := os.Stat(path); if err != nil { return DockerSocketIdentity{}, err }; stat, ok := info.Sys().(*syscall.Stat_t); if !ok { return DockerSocketIdentity{}, errors.New("docker socket identity unavailable") }; return DockerSocketIdentity{Device: uint64(stat.Dev), Inode: stat.Ino, Mode: uint32(stat.Mode), UID: stat.Uid}, nil },
		}
...
func loadPinnedWorkerForClaim(claim LeaseClaim) (ed25519.PrivateKey, error) {
	...
	if err != nil { return nil, errors.New("local swarm worker pinned key unavailable") }
	defer clear(loaded.Plaintext)
	if loaded.KeyGeneration.RecordDigest != claim.Candidate.GenerationPin.RecordDigest || loaded.KeyGeneration.DescriptorDigest != claim.Candidate.DescriptorDigest || loaded.Identity.Kind != "aid" { clear(loaded.PrivateKey); return nil, errors.New("local swarm worker pinned key mismatch") }
	public := loaded.PrivateKey.Public().(ed25519.PublicKey)
	encoded, der, err := publicKeySPKI(public)
	if err != nil || encoded != claim.Candidate.PublicKeySPKI || aidFromSPKI(der) != claim.Candidate.AID { clear(loaded.PrivateKey); return nil, errors.New("local swarm worker pinned key mismatch") }
...
func localWorkerReceipt(state SwarmState, claim LeaseClaim, result StagedArtifact, auxiliary []StagedArtifact, key ed25519.PrivateKey) (StagedReceipt, error) {
	...
	if stepIndex < 0 { return StagedReceipt{}, errors.New("local swarm worker step missing") }
	dependencies, err := committedDependencyTriples(state, stepIndex); if err != nil { return StagedReceipt{}, err }
	dependencyList := make([]receiptDependencyV2, 0, len(dependencies))
	for _, stepID := range state.Spec.Steps[stepIndex].DependsOn { dependencyList = append(dependencyList, receiptDependencyV2{StepID: stepID, Artifact: dependencies[stepID]}) }
	...
	decoder := json.NewDecoder(bytes.NewReader(bodyRaw)); decoder.UseNumber()
	if err := decoder.Decode(&normalizedBody); err != nil { return StagedReceipt{}, errors.New("local swarm worker receipt invalid") }
	if ensureSwarmJSONEOF(decoder) != nil { return StagedReceipt{}, errors.New("local swarm worker receipt invalid") }
	signed := signBodyWithKey(key, normalizedBody, "signature")
	raw, err := canonicalJSON(signed); if err != nil { return StagedReceipt{}, err }
...
func RunLocalSwarmWorker(ctx context.Context, request SwarmWorkerRequest, deps LocalSwarmWorkerDeps) (SwarmWorkerResult, error) {
	if err := request.validate(); err != nil { return SwarmWorkerResult{}, err }
	deps, err := deps.normalized(); if err != nil { return SwarmWorkerResult{}, err }
	journal, err := OpenSwarmJournal(request.StorageRoot, request.SwarmID); if err != nil { return SwarmWorkerResult{}, err }
	state, claim, err := exactLiveClaim(journal, request); if err != nil { return SwarmWorkerResult{}, err }
	profile, err := localWorkerProfile(claim.Candidate); if err != nil { return SwarmWorkerResult{}, err }
	if deps.Adapter == nil {
		deps.Adapter, err = localWorkerAdapter(claim.Candidate.RuntimeKind)
		if err != nil { return SwarmWorkerResult{}, err }
	}
	key, err := loadPinnedWorkerForClaim(claim); if err != nil { return SwarmWorkerResult{}, err }
	defer clear(key)
	execution, stopHeartbeat, heartbeatErrs := StartLeaseHeartbeat(ctx, journal, claim, deps.LeaseTTL, deps.Heartbeat, deps.Now)
	defer stopHeartbeat()
	runRequest, err := validateDockerWorkerProfile(*profile.Docker); if err != nil { return SwarmWorkerResult{}, err }
	runResult, err := deps.Adapter.Run(execution, runRequest)
	if err != nil { return SwarmWorkerResult{}, errors.New("local swarm worker container execution failed") }
	select { case err := <-heartbeatErrs: if err != nil { return SwarmWorkerResult{}, err }; default: }
	if err := execution.Err(); err != nil { return SwarmWorkerResult{}, errors.New("local swarm worker lease lost") }
	if len(runResult.Result) == 0 || runResult.MediaType == "" { return SwarmWorkerResult{}, errors.New("local swarm worker result invalid") }
	result, err := StageArtifact(journal, runResult.Result); if err != nil { return SwarmWorkerResult{}, err }
	auxiliary := []StagedArtifact{}
	if len(runResult.Transcript) != 0 { if runResult.TranscriptMediaType == "" { return SwarmWorkerResult{}, errors.New("local swarm worker transcript invalid") }; transcript, err := StageArtifact(journal, runResult.Transcript); if err != nil { return SwarmWorkerResult{}, err }; auxiliary = append(auxiliary, transcript) }
	...
	state, claim, err = exactLiveClaim(journal, request); if err != nil { return SwarmWorkerResult{}, err }
	receipt, err := localWorkerReceipt(state, claim, result, auxiliary, key); if err != nil { return SwarmWorkerResult{}, err }
	if _, err := CommitReceipt(journal, ReceiptCommit{Claim: claim, Receipt: receipt, Result: result, Auxiliary: auxiliary}, deps.Now().UTC()); err != nil { return SwarmWorkerResult{}, err }
...
func localSwarmWorkerMain(in io.Reader, out io.Writer, deps LocalSwarmWorkerDeps) error {
	raw, err := io.ReadAll(io.LimitReader(in, localSwarmWorkerMaxMessageBytes+1)); if err != nil || len(raw) > localSwarmWorkerMaxMessageBytes { return errors.New("local swarm worker input invalid") }
	request, err := ParseSwarmWorkerRequest(raw); if err != nil { return err }
	ctx, stop := signal.NotifyContext(context.Background(), syscall.SIGINT, syscall.SIGTERM)
	defer stop()
	result, err := RunLocalSwarmWorker(ctx, request, deps); if err != nil { return err }
	encoded, err := result.MarshalCanonical(); if err != nil { return err }
...
func setLocalSwarmWorkerDeps(deps LocalSwarmWorkerDeps) { localSwarmWorkerDepsMu.Lock(); localSwarmWorkerDeps = deps; localSwarmWorkerDepsMu.Unlock() }
func getLocalSwarmWorkerDeps() LocalSwarmWorkerDeps { localSwarmWorkerDepsMu.RLock(); defer localSwarmWorkerDepsMu.RUnlock(); return localSwarmWorkerDeps }
