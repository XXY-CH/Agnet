...
func TestLocalWorkerRequestCanonicalBoundedAndSecretFree(t *testing.T) {
	...
	if !bytes.Equal(encoded, []byte(`{"format":"agnet-local-swarm-worker-request/v1","storage_root":"/private/swarm","swarm_id":"swarm://test/worker","step_id":"prepare","owner":"worker-a","fence":7}`)) {
		t.Fatalf("request = %s", encoded)
	}
	if strings.Contains(string(encoded), "passphrase") || strings.Contains(string(encoded), "secret") {
		t.Fatalf("request exposes secret surface: %s", encoded)
	}
	...
	if _, err := oversized.MarshalCanonical(); err == nil {
...
func TestLocalWorkerLauncherUsesPipesOnly(t *testing.T) {
	...
	if err == nil {
		t.Fatal("false helper unexpectedly succeeded")
	}
	if strings.Contains(err.Error(), "passphrase") {
		t.Fatalf("launcher error leaks secret surface: %v", err)
	}
}
...
func TestLocalWorkerLauncherRejectsForgedChildMetadata(t *testing.T) {
	...
	if _, _, err := RecordNextReadyWave(journal, now.Add(time.Millisecond)); err != nil {
		t.Fatal(err)
	}
	...
	if _, err := RunLocalSwarmWorker(context.Background(), request, LocalSwarmWorkerDeps{Adapter: localWorkerTestAdapter{result: DockerRunResult{Result: []byte("authoritative result"), MediaType: "text/plain"}}, Now: time.Now, LeaseTTL: 300 * time.Millisecond, Heartbeat: 50 * time.Millisecond}); err != nil {
		t.Fatal(err)
	}
	...
	if _, err := launcher.Launch(context.Background(), request); err == nil {
		t.Fatal("accepted forged canonical child metadata")
	}
}
...
type localWorkerTestAdapter struct {
...
type localWorkerCancellationAdapter struct {
	startedPath string
	cleanedPath string
}

func (adapter localWorkerCancellationAdapter) Run(ctx context.Context, _ DockerRunRequest) (DockerRunResult, error) {
	if err := os.WriteFile(adapter.startedPath, []byte("started"), 0o600); err != nil {
		return DockerRunResult{}, err
	}
	<-ctx.Done()
	if err := os.WriteFile(adapter.cleanedPath, []byte("cleaned"), 0o600); err != nil {
		return DockerRunResult{}, err
	}
	return DockerRunResult{}, ctx.Err()
}
...
func TestLocalWorkerLauncherCancellationReachesChild(t *testing.T) {
	...
	for {
		...
		if time.Now().After(deadline) {
			t.Fatal("child adapter did not start")
		}
	...
	if _, err := os.Stat(cleanedPath); err != nil {
		t.Fatalf("child cleanup did not finish: %v", err)
	}
	...
	if state.Steps[0].Status != SwarmStepStatusRunning {
		t.Fatalf("cancelled child committed step: %#v", state.Steps[0])
	}
	for _, entry := range entries {
		if entry.Kind == "receipt.committed" {
			t.Fatal("cancelled child appended receipt commitment")
		}
	}
}

func TestLocalWorkerLauncherCancellationChildProcess(t *testing.T) {
	if os.Getenv("AGNET_SWARM_WORKER_CANCELLATION_CHILD") == "" {
		return
	}
	...
	if err := localSwarmWorkerMain(os.Stdin, os.Stdout, deps); err == nil {
		os.Exit(2)
	}
	os.Exit(0)
}
...
func TestLocalWorkerCommitsDockerResultAndTranscript(t *testing.T) {
	...
	if result.Format != localSwarmWorkerResultFormat || result.ReceiptDigest == "" {
	...
	if err == nil || state.Version != 0 {
		t.Fatal("committed worker retained a live lease")
	}
	entries, err := journal.Replay()
	if err != nil {
		t.Fatal(err)
	}
	committed, err := ReduceSwarmEntries(entries)
	if err != nil || committed.Steps[0].Status != SwarmStepStatusCompleted {
...
func TestLocalWorkerHeartbeatPreventsExpiry(t *testing.T) {
	journal := newTestSwarmJournal(t)
	now := time.Now().UTC()
	if _, err := OpenVerifiedSwarm(journal, reducerTestDurableSpec(t), now); err != nil {
		t.Fatal(err)
	}
	if _, _, err := RecordNextReadyWave(journal, now.Add(time.Millisecond)); err != nil {
		t.Fatal(err)
	}
	dispatch, err := ClaimReadyWave(journal, "heartbeat", now.Add(80*time.Millisecond), now.Add(2*time.Millisecond))
	if err != nil {
		t.Fatal(err)
	}
	...
	state, err := ExpireLeases(journal, time.Now().UTC())
	if err != nil {
		t.Fatal(err)
	}
	if len(state.Leases) != 1 {
		t.Fatalf("heartbeat lease expired: %#v", state.Leases)
	}
}
