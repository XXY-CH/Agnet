...
func loadU29Vector(t *testing.T, name string) swarmParityVector {
	...
	if err := json.Unmarshal(raw, &vector); err != nil { t.Fatal(err) }
	return vector
}
...
func verifyU29Vector(t *testing.T, vector swarmParityVector) {
	...
	for index, entry := range vector.Journal {
		...
		if err != nil { t.Fatalf("canonical payload %s entry %d: %v", vector.Origin, index+1, err) }
		entry.Payload = canonicalPayload
		vector.Journal[index] = entry
		if err := validateSwarmJournalEntry(entry, uint64(index+1), previousVersion, previousHash); err != nil { t.Fatalf("Go rejected %s entry %d: %v", vector.Origin, index+1, err) }
		previousHash, previousVersion = entry.Hash, entry.StateVersion
	}
	if previousHash != vector.Expected.Head || previousVersion != vector.Expected.StateVersion { t.Fatalf("Go replay head/version mismatch: %q/%d", previousHash, previousVersion) }
	state, err := ReduceSwarmEntries(vector.Journal)
	if err != nil { t.Fatalf("Go reducer rejected %s vector: %v", vector.Origin, err) }
	if state.Version != vector.Expected.StateVersion || state.Spec.SwarmID != vector.Expected.SwarmID || string(state.Status) != vector.Expected.Status || state.StoredClose.Digest != vector.Expected.CloseDigest || state.StoredDisband.Digest != vector.Expected.DisbandDigest || state.OutputVerification == nil || state.OutputVerification.ProofDigest != vector.Expected.ProofDigest { t.Fatalf("Go reducer state mismatch: %#v", state) }
	if !bytes.Equal(state.StoredClose.Bytes, decodeU29B64(t, vector.Evidence.Close)) || !bytes.Equal(state.StoredDisband.Bytes, decodeU29B64(t, vector.Evidence.Disband)) { t.Fatal("stored close/disband bytes differ from vector evidence") }
	proof := decodeU29B64(t, vector.Evidence.Proof)
	if !bytes.Equal(proof, decodeU29OutputProof(t, vector.Journal)) { t.Fatal("stored proof bytes differ from vector evidence") }
	if state.OutputVerification.TrustInputsDigest != vector.Evidence.TrustInputsDigest { t.Fatal("output verification did not bind frozen trust") }
	for index, entry := range vector.Journal {
		switch entry.Kind {
		case "close.stored":
			stored, err := storedSwarmCloseFromEntry(entry); if err != nil { t.Fatal(err) }
			prior, err := ReduceSwarmEntries(vector.Journal[:index]); if err != nil { t.Fatal(err) }
			if err := verifyJournalCloseV2(stored.Bytes, prior, vector.Journal[:index]); err != nil { t.Fatalf("close verifier rejected %s: %v", vector.Origin, err) }
			mutated := append([]byte(nil), stored.Bytes...); mutated[len(mutated)-1] ^= 1
			if err := verifyJournalCloseV2(mutated, prior, vector.Journal[:index]); err == nil { t.Fatal("close verifier accepted close/proof mismatch") }
		case "output.verified":
			...
			prior, err := ReduceSwarmEntries(vector.Journal[:index]); if err != nil { t.Fatal(err) }
			legacy := payload; legacy.CloseDigest = strings.Repeat("0", 64)
			if _, err := ReduceSwarmEntry(prior, SwarmJournalEntry{Format: swarmJournalFormat, Sequence: entry.Sequence, PriorStateVersion: entry.PriorStateVersion, StateVersion: entry.StateVersion, Kind: entry.Kind, Payload: mustCanonicalU29Payload(t, legacy), Timestamp: entry.Timestamp, PrevHash: entry.PrevHash}); err == nil { t.Fatal("reducer accepted legacy digest substitution") }
			mutatedTrust := payload
			mutatedTrust.TrustInputsDigest = strings.Repeat("f", 64)
			if _, err := ReduceSwarmEntry(prior, SwarmJournalEntry{Format: swarmJournalFormat, Sequence: entry.Sequence, PriorStateVersion: entry.PriorStateVersion, StateVersion: entry.StateVersion, Kind: entry.Kind, Payload: mustCanonicalU29Payload(t, mutatedTrust), Timestamp: entry.Timestamp, PrevHash: entry.PrevHash}); err == nil { t.Fatal("reducer accepted frozen trust mutation") }
		case "swarm.disbanded":
			stored, err := storedSwarmDisbandFromEntry(entry); if err != nil { t.Fatal(err) }
			prior, err := ReduceSwarmEntries(vector.Journal[:index]); if err != nil { t.Fatal(err) }
			if err := verifySwarmDisbandAgainstState(stored.Bytes, prior); err != nil { t.Fatalf("disband verifier rejected %s: %v", vector.Origin, err) }
			var wire swarmDisbandWire; if err := json.Unmarshal(stored.Bytes, &wire); err != nil { t.Fatal(err) }; wire.DisbandSignature = strings.Repeat("A", len(wire.DisbandSignature)); bad, err := canonicalJSON(wire); if err != nil { t.Fatal(err) }
			if err := verifySwarmDisbandAgainstState(bad, prior); err == nil { t.Fatal("disband verifier accepted bad signature") }
		}
	}
}

func decodeU29B64(t *testing.T, value string) []byte {
	t.Helper(); raw, err := base64.RawURLEncoding.DecodeString(value); if err != nil || base64.RawURLEncoding.EncodeToString(raw) != value { t.Fatal("vector evidence base64 invalid") }; return raw
}

func decodeU29OutputProof(t *testing.T, entries []SwarmJournalEntry) []byte {
	t.Helper()
	for _, entry := range entries { if entry.Kind == "output.verified" { var payload outputVerifiedPayload; if err := decodeStrictSwarmPayload(entry.Payload, &payload); err != nil { t.Fatal(err) }; return decodeU29B64(t, payload.Proof) } }
	t.Fatal("output.verified missing"); return nil
}

func mustCanonicalU29Payload(t *testing.T, value any) json.RawMessage {
	t.Helper(); raw, err := canonicalSwarmPayload(value); if err != nil { t.Fatal(err) }; return raw
}

func TestGenerateU29DurableVectors(t *testing.T) {
	if os.Getenv("UPDATE_U29_VECTORS") != "1" { return }
	vector := buildU29Vector(t, "go")
	raw, err := json.MarshalIndent(vector, "", "  "); if err != nil { t.Fatal(err) }
...
func buildU29Vector(t *testing.T, origin string) swarmParityVector {
	...
	if _, err := EnsureDisband(journal); err != nil { t.Fatal(err) }
	entries := mustReplaySwarm(t, journal)
	normalizeU29OpenEntry(t, entries)
	state, err := ReduceSwarmEntries(entries); if err != nil { t.Fatal(err) }
	...
	for _, step := range state.Spec.Steps { for _, candidate := range step.Candidates { var descriptor map[string]any; if err := json.Unmarshal([]byte(candidate.Descriptor), &descriptor); err != nil { t.Fatal(err) }; result.Evidence.FrozenWorkers = append(result.Evidence.FrozenWorkers, descriptor) } }
	return result
}

func normalizeU29OpenEntry(t *testing.T, entries []SwarmJournalEntry) {
	t.Helper(); if len(entries) == 0 { t.Fatal("empty vector journal") }
	var opened swarmOpenedPayload; if err := decodeStrictSwarmPayload(entries[0].Payload, &opened); err != nil { t.Fatal(err) }
	...
	for index := range entries { entries[index].PrevHash = previous; hash, err := swarmJournalEntryHash(entries[index]); if err != nil { t.Fatal(err) }; entries[index].Hash = hash; previous = hash }
}
