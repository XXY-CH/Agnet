...
func swarmMutationAllowed(state SwarmState) error {
	if state.Status == SwarmStatusDisbanded { return ErrSwarmDisbanded }
	return nil
}
...
func buildSwarmDisbandLocked(state SwarmState) (StoredSwarmDisband, error) {
	if state.Version == 0 || state.Status != SwarmStatusCompleted || state.OutputVerification == nil || state.StoredClose.Digest == "" { return StoredSwarmDisband{}, errors.New("swarm disband requires completed output verification") }
...
func EnsureDisband(journal *SwarmJournal) (StoredSwarmDisband, error) {
	...
	err := journal.WithLockedReplay(func(entries []SwarmJournalEntry) error {
		...
		for index, entry := range entries {
			if entry.Kind != "swarm.disbanded" { continue }
			...
			if err != nil { return err }
			if !bytes.Equal(stored.Bytes, candidate.Bytes) || stored.Digest != candidate.Digest { return errors.New("swarm disband conflicts with stored disband") }
			result = stored
			return nil
		}
		...
		if err != nil { return err }
		if state.Version == math.MaxUint64 || len(entries) == 0 { return errors.New("swarm disband state invalid") }
		...
		if err != nil { return err }
		if err := journal.validateTypedStateIdentity(next); err != nil { return err }
		if err := journal.appendLocked(entry); err != nil { return err }
		result = candidate
		return nil
	})
	return result, err
...
func VerifySwarmDisband(raw []byte, journal *SwarmJournal) error {
	...
	return journal.WithLockedReplay(func(entries []SwarmJournalEntry) error {
		for index, entry := range entries {
			...
			stored, err := storedSwarmDisbandFromEntry(entry)
			if err != nil { return err }
			if !bytes.Equal(raw, stored.Bytes) { return errors.New("swarm disband bytes differ from stored record") }
			prior, err := ReduceSwarmEntries(entries[:index])
			if err != nil { return err }
...
func storedSwarmDisbandFromEntry(entry SwarmJournalEntry) (StoredSwarmDisband, error) {
	...
	if err := decodeStrictSwarmPayload(entry.Payload, &payload); err != nil || payload.SchemaVersion != swarmStateSchemaVersion || !isHexDigest(payload.Digest) || payload.Disband == "" { return StoredSwarmDisband{}, errors.New("stored swarm disband invalid") }
	raw, err := base64.RawURLEncoding.DecodeString(payload.Disband)
	if err != nil || base64.RawURLEncoding.EncodeToString(raw) != payload.Disband || digestBytesHex(raw) != payload.Digest { return StoredSwarmDisband{}, errors.New("stored swarm disband invalid") }
...
func verifySwarmDisbandAgainstState(raw []byte, state SwarmState) error {
	if state.Version == 0 || state.Status != SwarmStatusCompleted || state.OutputVerification == nil || state.StoredClose.Digest == "" || !json.Valid(raw) || validateSwarmJSONNoDuplicateFields(raw) != nil { return errors.New("swarm disband invalid") }
	...
	if err != nil || base64.RawURLEncoding.EncodeToString(signature) != wire.DisbandSignature || len(signature) != ed25519.SignatureSize { return errors.New("swarm disband signature invalid") }
	planDigest, graphDigest, err := swarmCloseBindingDigests(state.Spec)
	if err != nil || wire.PlanDigest != planDigest || wire.ExecutionGraphDigest != graphDigest { return errors.New("swarm disband binding invalid") }
	if len(state.Spec.LocalAuthority) == 0 || verifyZoneDescriptor(state.Spec.LocalAuthority) != nil { return errors.New("swarm disband authority invalid") }
	key, _, err := publicKey(state.Spec.LocalAuthority)
	if err != nil { return errors.New("swarm disband authority invalid") }
	body := swarmDisbandBody(wire); body["disband_signature"] = wire.DisbandSignature
	if err := verifyMapSignature(key, body, "disband_signature"); err != nil { return errors.New("swarm disband signature invalid") }
	return nil
}
