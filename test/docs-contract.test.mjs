...
test("product README presents the product surface and authoritative documentation", async () => {
  ...
  for (const document of [
    ...
    "docs/agent-space-ultimate-vision.md",
  ]) {
    assert.match(
      readme,
  ...
  assert.doesNotMatch(readme, /^## Protocol reference index$/m);
  assert.doesNotMatch(readme, /docs\/v11\.3-boundary\.md/);
...
test("v10 public docs agree that the proof milestone is closed", async () => {
  const [roadmap, boundary] = await Promise.all([
...
test("v11 public docs include task id token validation", async () => {
  const [roadmap, draft, status, boundary] = await Promise.all([
    readFile("docs/v11-roadmap.md", "utf8"),
    readFile("docs/asp-core-draft.md", "utf8"),
...
test("v11 public docs include artifact closure task evidence parity", async () => {
  ...
  assert.match(
    status,
    /optional supplied-task `task_digest` verification across Node receipt\/artifact CLIs/,
  );
...
test("v11 public docs include Go receipt CLI task evidence verification", async () => {
  ...
  assert.match(
    draft,
    /--verify-receipt <receipt\.json> \[--verify-task <task\.json>\]/,
  );
...
test("v11 public docs include FED_RECEIPT frame type validation", async () => {
  ...
  assert.match(
    draft,
    /frame is an object whose `frame\.type` is `FED_RECEIPT`/,
  );
  assert.match(
    status,
...
test("v11 public docs include FED_TASK_OPEN frame type validation", async () => {
  ...
  assert.match(
    draft,
    /frame is an object whose `frame\.type` is `FED_TASK_OPEN`/,
  );
  assert.match(
    status,
    /Node `FED_TASK_OPEN` and `FED_RECEIPT` frame object\/type.*validation/,
  );
  assert.match(boundary, /wrong protocol type/);
});
...
test("v11 public docs include FED_SWARM_CLOSE duplicate step validation", async () => {
  ...
  assert.match(
    status,
    /structural close-frame\/close-zone\/close-proof\/close-signature\/step-receipt object\/identity\/task-id\/uniqueness\/NUL checks/,
  );
...
test("v11 public docs include FED_SWARM_CLOSE Swarm identity presence", async () => {
  ...
  assert.match(
    draft,
    /checks the close frame object and type, .*signing Zone object and descriptor, close proof object, close signature presence and verification/,
  );
  assert.match(
    status,
    /structural close-frame\/close-zone\/close-proof\/close-signature\/step-receipt object\/identity\/task-id\/uniqueness\/NUL checks/,
...
test("v11 public docs include task and receipt Zone descriptor presence", async () => {
  ...
  assert.match(
    draft,
    /origin Zone descriptor is present as an object and verifies/,
  );
  assert.match(
    draft,
    /signing Zone descriptor is present as an object and trusted/,
  );
  assert.match(
    status,
    /Node `FED_TASK_OPEN` and `FED_RECEIPT` frame object\/type.*Zone descriptor presence.*validation/,
  );
...
test("v11 public docs include task and receipt payload object presence", async () => {
  ...
  assert.match(
    status,
    /Node `FED_TASK_OPEN` and `FED_RECEIPT` frame object\/type, Zone descriptor presence, payload object presence, and trusted Zone store presence validation/,
  );
...
test("v11 public docs include Node trusted Zone store presence", async () => {
  ...
  assert.match(
    draft,
    /trusted Zone store is present for signing and origin Zone lookup/,
  );
  assert.match(
    draft,
    /trusted Zone store presence, signing Zone object and descriptor/,
  );
...
test("v11 public docs include FED_TASK_OPEN worker context presence", async () => {
  ...
  assert.match(
    draft,
    /local worker descriptor context is present as an object/,
  );
...
test("v11 public docs include FED_TASK_OPEN worker descriptor identity", async () => {
  ...
  assert.match(
    status,
    /worker descriptor context presence validation and worker descriptor identity validation/,
  );
...
test("v11 public docs include FED_RECEIPT worker descriptor identity", async () => {
  ...
  assert.match(
    draft,
    /worker descriptor identity verifies before receipt identity and signature checks/,
  );
  assert.match(status, /状态：v12 closed at v12\.45/);
  assert.match(
    status,
    /Node `FED_RECEIPT` worker descriptor identity validation/,
  );
...
test("v11 public docs include descriptor public key presence", async () => {
  ...
  assert.match(
    draft,
    /public_key_spki` is missing before handing the descriptor to Node crypto parsing/,
  );
  assert.match(status, /状态：v12 closed at v12\.45/);
...
test("v11 public docs include object signature fail-closed verification", async () => {
  ...
  assert.match(
    draft,
    /object signature verification returns false for missing, empty, or non-string signatures/,
  );
  assert.match(status, /状态：v12 closed at v12\.45/);
...
test("v11 public docs include Zone descriptor object presence", async () => {
  ...
  assert.match(
    draft,
    /Zone descriptor object presence before reading descriptor fields/,
  );
  assert.match(status, /状态：v12 closed at v12\.45/);
...
test("v11 public docs include did:key input presence", async () => {
  ...
  assert.match(
    draft,
    /did:key` bridge helpers reject missing descriptor\/public-key and DID string inputs/,
  );
  assert.match(status, /状态：v12 closed at v12\.45/);
...
test("v11 public docs include artifact manifest object presence", async () => {
  ...
  assert.match(
    draft,
    /artifact manifest helpers reject missing receipt and manifest objects/,
  );
  assert.match(status, /状态：v12 closed at v12\.45/);
...
test("v11 public docs include credential object presence", async () => {
  ...
  assert.match(
    draft,
    /capability credential helpers reject missing credential and status proof objects/,
  );
  assert.match(status, /状态：v12 closed at v12\.45/);
...
test("v11 public docs include rotation proof object presence", async () => {
  ...
  assert.match(
    draft,
    /rotation and alias rebinding proof verifiers reject missing proof and descriptor objects/,
  );
  assert.match(status, /状态：v12 closed at v12\.45/);
...
test("v11 public docs include Zone binding object presence", async () => {
  ...
  assert.match(
    draft,
    /Zone binding verifier rejects missing binding context and descriptor objects/,
  );
  assert.match(status, /状态：v12 closed at v12\.45/);
...
test("v11 public docs include Zone revocation object presence", async () => {
  ...
  assert.match(
    draft,
    /Zone revocation verifiers reject missing revocation context, descriptor, and revocation-list objects/,
  );
  assert.match(status, /状态：v12 closed at v12\.45/);
...
test("v11 public docs include descriptor body object presence validation", async () => {
  ...
  assert.match(
    draft,
    /Descriptor body helpers MUST receive descriptor objects/,
  );
  assert.match(status, /状态：v12 closed at v12\.45/);
...
test("v11 public docs include proof verifier malformed descriptor fail-closed validation", async () => {
  ...
  assert.match(
    draft,
    /boolean proof verifiers return false for malformed descriptor inputs/,
  );
  assert.match(status, /状态：v12 closed at v12\.45/);
  assert.match(
    status,
    /Node proof verifier malformed descriptor fail-closed validation/,
  );
  assert.match(boundary, /descriptor public key missing/);
  assert.match(boundary, /zone descriptor missing/);
});
...
test("v11 public docs include local artifact URI boundary validation", async () => {
  ...
  assert.match(
    draft,
    /Local artifact byte verification MUST reject missing, non-`artifact:\/\/local\/`, or path-escaping manifest URIs/,
  );
  assert.match(status, /状态：v12 closed at v12\.45/);
...
test("v11 public docs include Go artifact digest path boundary validation", async () => {
  ...
  assert.match(
    draft,
    /reject malformed manifest `sha256` values before constructing digest-addressed sidecar or mirror paths/,
  );
  assert.match(status, /状态：v12 closed at v12\.45/);
...
test("v11 public docs include receipt artifact digest shape validation", async () => {
  ...
  assert.match(
    draft,
    /Receipt artifact manifest verification MUST reject malformed manifest `uri` and `sha256` values/,
  );
  assert.match(status, /状态：v12 closed at v12\.45/);
...
test("v11 public docs include receipt artifact size shape validation", async () => {
  ...
  assert.match(
    draft,
    /MUST reject negative or non-integer manifest `size` values/,
  );
  assert.match(status, /状态：v12 closed at v12\.45/);
...
test("v11 public docs include Go artifact media type shape validation", async () => {
  ...
  assert.match(
    draft,
    /Go receipt and audit artifact manifest verification MUST reject non-string manifest `media_type` values/,
  );
  assert.match(status, /状态：v12 closed at v12\.45/);
...
test("v11 public docs include Go artifact manifest hash shape validation", async () => {
  ...
  assert.match(
    draft,
    /Go receipt and audit artifact manifest verification MUST reject non-string manifest `manifest_hash` values/,
  );
  assert.match(status, /状态：v12 closed at v12\.45/);
...
test("v11 public docs include Go artifact list shape validation", async () => {
  ...
  assert.match(
    draft,
    /MUST reject malformed `artifact_refs` and `artifact_manifests` list entries/,
  );
  assert.match(status, /状态：v12 closed at v12\.45/);
...
test("v11 public docs include Go artifact mirror index shape validation", async () => {
  ...
  assert.match(
    draft,
    /MUST match index fields against receipt artifact manifest fields without string coercion/,
  );
  assert.match(status, /状态：v12 closed at v12\.45/);
  assert.match(
    status,
    /Go artifact mirror index exact field matching validation/,
  );
...
test("v11 public docs include Go artifact mirror index entry validation", async () => {
  ...
  assert.match(
    draft,
    /MUST reject non-object `objects\.ndjson` entries such as `null`/,
  );
  assert.match(status, /状态：v12 closed at v12\.45/);
...
test("v11 public docs include Go artifact mirror index digest validation", async () => {
  ...
  assert.match(
    draft,
    /MUST reject index entries whose `sha256` field is missing or not a 64-hex digest/,
  );
  assert.match(status, /状态：v12 closed at v12\.45/);
...
test("v11 public docs include Go artifact mirror index manifest hash validation", async () => {
  ...
  assert.match(
    draft,
    /MUST reject present `manifest_hash` values that are not 64-hex digests/,
  );
...
test("v11 public docs include Go artifact mirror index AFP validation", async () => {
  ...
  assert.match(
    draft,
    /Present string `afp` values MUST equal `afp:sha256:<sha256>`/,
  );
...
test("v11 public docs include Go artifact mirror index size validation", async () => {
  ...
  assert.match(
    draft,
    /MUST reject present `size` values that are not non-negative integers/,
  );
...
test("v11 public docs include Go artifact mirror index media type validation", async () => {
  ...
  assert.match(
    draft,
    /MUST reject present `media_type` values that are not strings/,
  );
  assert.match(status, /状态：v12 closed at v12\.45/);
...
test("v11 public docs include Go artifact mirror index AFP type validation", async () => {
  ...
  assert.match(
    draft,
    /MUST reject present non-string `afp` values before comparing AFP strings/,
  );
  assert.match(status, /状态：v12 closed at v12\.45/);
...
test("v11 public docs include Go Swarm dependency list shape validation", async () => {
  ...
  assert.match(
    draft,
    /MUST reject malformed `after` and `input_artifacts` list entries/,
  );
  assert.match(status, /状态：v12 closed at v12\.45/);
...
test("v11 public docs include Go receipt approval list shape validation", async () => {
  ...
  assert.match(
    draft,
    /rejects malformed `approvals` and `approval_grants` list entries/,
  );
  assert.match(status, /状态：v12 closed at v12\.45/);
...
test("v11 public docs include Go receipt checkpoint list shape validation", async () => {
  ...
  assert.match(
    draft,
    /rejects malformed `checkpoint_refs` and `checkpoints` list entries/,
  );
  assert.match(status, /状态：v12 closed at v12\.45/);
...
test("v11 public docs include Go runtime checkpoint lookup list shape validation", async () => {
  ...
  assert.match(
    draft,
    /checkpoint lookup for resume rejects malformed receipt `checkpoint_refs` and `checkpoints` list entries/,
  );
  assert.match(status, /状态：v12 closed at v12\.45/);
...
test("v11 public docs include Go receipt artifact lookup list shape validation", async () => {
  ...
  assert.match(
    draft,
    /receipt artifact lookup rejects malformed `artifact_refs` and `artifact_manifests` list entries/,
  );
  assert.match(status, /状态：v12 closed at v12\.45/);
...
test("v11 public docs include Go FED_SWARM_OPEN after list shape validation", async () => {
  ...
  assert.match(
    draft,
    /FED_SWARM_OPEN` execution MUST reject malformed step `after` list entries/,
  );
  assert.match(status, /状态：v12 closed at v12\.45/);
...
test("v11 public docs include Go queue grant scope list shape validation", async () => {
  ...
  assert.match(
    draft,
    /queue action grants reject malformed signed `scope\.actions` list entries/,
  );
  assert.match(status, /状态：v12 closed at v12\.45/);
...
test("v11 public docs include Go task write scope list shape validation", async () => {
  ...
  assert.match(
    draft,
    /policy enforcement rejects malformed signed task `scope\.write` list entries/,
  );
  assert.match(status, /状态：v12 closed at v12\.45/);
  assert.match(
    status,
    /Go signed task write\/data-domain list shape validation/,
  );
...
test("v11 public docs include Go task data domains list shape validation", async () => {
  ...
  assert.match(
    draft,
    /policy enforcement rejects malformed signed task `scope\.data_domains` list entries/,
  );
  assert.match(status, /状态：v12 closed at v12\.45/);
  assert.match(
    status,
...
test("v11 public docs include Go worker approval required list shape validation", async () => {
  ...
  assert.match(
    draft,
    /worker policy `approval_required` list entries before tool approval gates can be skipped/,
  );
  assert.match(status, /状态：v12 closed at v12\.45/);
...
test("v11 public docs include Go receipt policy scope list shape validation", async () => {
  ...
  assert.match(
    draft,
    /receipt verification rejects malformed `policy_scope` `write`, `tools`, `data_domains`, and `approval_required` list entries/,
  );
  assert.match(status, /状态：v12 closed at v12\.45/);
...
test("v11 public docs include Go receipt policy scope scalar shape validation", async () => {
  ...
  assert.match(
    draft,
    /receipt verification rejects malformed `policy_scope\.network` and `policy_scope\.expires_at` scalar fields/,
  );
  assert.match(status, /状态：v12 closed at v12\.45/);
  assert.match(
    status,
    /receipt policy scope list shape validation and scalar shape validation/,
  );
...
test("v11 public docs include Go receipt task id token validation", async () => {
  ...
  assert.match(
    draft,
    /Node and Go receipt verification reject unsafe signed receipt `task_id` values/,
  );
  assert.match(status, /状态：v12 closed at v12\.45/);
  assert.match(
    status,
...
test("v11 public docs include Node receipt artifact URI shape validation", async () => {
  ...
  assert.match(
    draft,
    /Node and Go receipt artifact manifest verification MUST reject non-string manifest `uri` values/,
  );
  assert.match(status, /状态：v12 closed at v12\.45/);
...
test("v11 public docs include public transport receipt proof binding", async () => {
  ...
  assert.match(
    draft,
    /Go server-mode task receipts MAY include `transport_proof`/,
  );
  assert.match(status, /状态：v12 closed at v12\.45/);
  assert.match(status, /Go public-listen signed receipt transport proof/);
  assert.match(
    boundary,
    /transport_proof` binds `transport`, `listen_host`, `port`, and `public_transport`/,
  );
...
test("v12 public docs start with public proof bundle manifests", async () => {
  const [roadmap, status, boundary] = await Promise.all([
...
test("v12 public docs include proof bundle verifier command", async () => {
  ...
  assert.match(
    boundary,
    /delegates to the existing receipt artifact and Swarm close verifier paths/,
  );
...
test("v12 public docs include public proof summary bundle verification", async () => {
  ...
  assert.match(
    boundary,
    /public proof summary returns `proof_bundle_verify: "ok"`/,
  );
...
test("v12 public docs include proof bundle path safety", async () => {
  ...
  assert.match(
    boundary,
    /rejects empty, absolute, backslash-bearing, `\.` segment, and `\.\.` segment proof-file paths/,
  );
...
test("v12 public docs include proof bundle path preflight", async () => {
  ...
  assert.match(
    boundary,
    /validates `receipt_frame`, `trusted_zones`, `swarm_close_frame`, and `swarm_close_trusted_zones` before reading any of them/,
  );
...
test("v12 public docs include verifier CLI exact arity", async () => {
  ...
  assert.match(
    draft,
    /The verifier CLI commands reject unsupported extra positional arguments/,
  );
  assert.match(status, /状态：v12 closed at v12\.45/);
  assert.match(status, /v12.45-closeout-alignment/);
  assert.match(
    boundary,
    /Make every `asp-verify\.mjs` command reject extra positional CLI arguments/,
  );
  assert.match(boundary, /does not add option parsing/);
});
...
test("v12 public docs include proof bundle reachability scope ownership", async () => {
  ...
  assert.match(
    roadmap,
    /rejects bundle manifests that include `reachability_scope`/,
  );
  assert.match(
    draft,
    /rejects bundle manifests that supply their own `reachability_scope`/,
  );
  assert.match(status, /状态：v12 closed at v12\.45/);
  assert.match(status, /v12.45-closeout-alignment/);
...
test("v12 public docs include package artifact proof", async () => {
  ...
  assert.match(
    roadmap,
    /npm pack --json --pack-destination state\/package-proof/,
  );
...
test("v12 public docs include package proof digest", async () => {
  ...
  assert.match(
    roadmap,
    /computes `proof_digest` as `sha256\(canonical\(proof without proof_digest\)\)`/,
  );
  assert.match(draft, /v12\.45-protocol/);
...
test("v12 public docs include package proof verifier command", async () => {
  ...
  assert.match(
    roadmap,
    /verifies `proof_digest`, tarball `sha256`, and tarball size/,
  );
  ...
  assert.match(
    boundary,
    /verify the generated package proof manifest and tarball/,
  );
  assert.match(boundary, /does not sign the tarball or produce an SBOM/);
});
...
test("v12 public docs include package proof manifest object gate", async () => {
  ...
  assert.match(
    draft,
    /package proof verifier rejects `null` and array manifests/,
  );
  assert.match(status, /状态：v12 closed at v12\.45/);
  assert.match(status, /v12.45-closeout-alignment/);
...
test("v12 public docs include package proof manifest-relative tarball", async () => {
  ...
  assert.match(
    roadmap,
    /resolves safe tarball paths relative to the manifest file/,
  );
  assert.match(
    draft,
    /package proof verifier resolves safe tarball paths relative to the package proof manifest file/,
  );
  assert.match(status, /状态：v12 closed at v12\.45/);
  assert.match(status, /v12.45-closeout-alignment/);
...
test("v12 public docs include package proof npm digest verification", async () => {
  ...
  assert.match(
    roadmap,
    /checks `integrity` as the npm `sha512-<base64>` string/,
  );
...
test("v12 public docs include package proof verified metadata output", async () => {
  ...
  assert.match(
    roadmap,
    /returns verified `size`, `shasum`, and `integrity` fields/,
  );
  assert.match(
    draft,
    /returns the verified package name, version, filename, tarball path, size, npm shasum, npm integrity, ASP SHA-256, proof digest, and signer Agent ID/,
  );
  assert.match(status, /状态：v12 closed at v12\.45/);
  assert.match(status, /v12.45-closeout-alignment/);
...
test("v12 public docs include package proof filename binding", async () => {
  ...
  assert.match(
    roadmap,
    /requires `filename` to equal the final path segment of `tarball`/,
  );
  assert.match(
    draft,
    /requires `filename` to equal the final path segment of `tarball`/,
...
test("v12 public docs include package proof file list shape", async () => {
  ...
  assert.match(
    roadmap,
    /requires `files` to be a non-empty array of unique safe relative paths/,
  );
  assert.match(
    draft,
    /The `files` field MUST be a non-empty array of unique safe relative paths/,
  );
  assert.match(status, /状态：v12 closed at v12\.45/);
  assert.match(status, /v12.45-closeout-alignment/);
  ...
  assert.match(
    boundary,
    /not package signing, SBOM, or a tarball member proof/,
  );
});
...
test("v12 public docs include package proof manifest filename binding", async () => {
  ...
  assert.match(
    roadmap,
    /requires `manifest` to equal the final path segment of the verifier input path/,
  );
  assert.match(
    draft,
    /requires `manifest` to equal the final path segment of the verifier input path/,
  ...
  assert.match(
    boundary,
    /reject manifests whose `manifest` field does not match/,
  );
  assert.match(
    boundary,
    /not package signing, SBOM, or a tarball member proof/,
...
test("v12 public docs include package proof identity filename binding", async () => {
  ...
  assert.match(
    boundary,
    /reject manifests whose package `name` and `version` metadata do not match/,
  );
  assert.match(
    boundary,
    /not package signing, SBOM, or a tarball member proof/,
...
test("v12 public docs include Go canonical JSON HTML escape parity", async () => {
  ...
  assert.match(
    roadmap,
    /match Node canonical JSON for signed and digested values containing `<`, `>`, and `&`/,
  );
  assert.match(
    draft,
    /Go protocol signing, signature verification, and digest paths MUST use canonical JSON without HTML escaping/,
  );
  assert.match(status, /状态：v12 closed at v12\.45/);
  assert.match(status, /v12.45-closeout-alignment/);
  ...
  assert.match(
    boundary,
    /Node canonical JSON does not HTML-escape `<`, `>`, or `&`/,
  );
...
test("v12 public docs include external reachability evidence gate", async () => {
  ...
  assert.match(
    status,
    /proof-bundle external reachability evidence trust gate/,
  );
  assert.match(
    boundary,
    /external-host` requires a separately supplied trusted observer/,
  );
...
test("v12 public docs keep upper-layer demo outside the core line", async () => {
  ...
  assert.match(
    roadmap,
    /upper-layer demo\/orchestration work parked outside this repository/,
  );
...
test("v12 public docs include package proof ASP signature", async () => {
  ...
  assert.match(
    draft,
    /not npm registry signing, release transparency, package publish, or SBOM/,
  );
  assert.match(status, /状态：v12 closed at v12\.45/);
  assert.match(status, /v12.45-closeout-alignment/);
  ...
  assert.match(
    boundary,
    /Sign the local package proof manifest with an ASP agent identity/,
  );
...
test("v12 public docs include package proof metadata preflight", async () => {
  ...
  assert.match(
    draft,
    /rejects malformed packaged file lists before reading tarball bytes/,
  );
  assert.match(status, /状态：v12 closed at v12\.45/);
  assert.match(status, /v12.45-closeout-alignment/);
...
test("v12 public docs include package proof identity shape", async () => {
  ...
  assert.match(
    draft,
    /requires `name`, `version`, and `filename` to be non-empty strings/,
  );
  assert.match(status, /状态：v12 closed at v12\.45/);
  assert.match(status, /v12.45-closeout-alignment/);
...
test("v12 public docs include package proof byte metadata shape", async () => {
  ...
  assert.match(
    draft,
    /requires `shasum`, `integrity`, and `sha256` to be non-empty strings/,
  );
  assert.match(status, /状态：v12 closed at v12\.45/);
  assert.match(status, /v12.45-closeout-alignment/);
...
test("v12 public docs agree that the proof-surface milestone is closed", async () => {
  ...
  assert.match(
    boundary,
    /Close v12 as the externally consumable proof-surface milestone/,
  );
...
test("v13 public docs open the Ultimate-facing protocol milestone without scope drift", async () => {
  ...
  assert.match(
    roadmap,
    /v13 uses larger evidence gates instead of many tiny versions/,
  );
  ...
  assert.match(
    roadmap,
    /upper-layer demo\/master-agent orchestration stays outside this repository/,
  );
...
test("v13.1 public docs describe active reachability scope without hosted exit drift", async () => {
  ...
  assert.match(
    roadmap,
    /hostname listen hosts are out of scope for this slice/,
  );
  assert.match(
    draft,
    /reports `reachability_scope: "container-observer"` for `vantage: "container"`/,
  );
  assert.match(
    draft,
    /`reachability_scope: "external-host"` for `vantage: "external-host"` only when the signed receipt `listen_host` is a globally routable literal IP/,
  );
  assert.match(
    draft,
    /Observer-backed scopes return `reachability_observer_zid`/,
  );
  assert.match(status, /状态：v14 active at v14\.11/);
  assert.match(
    status,
    /v13\.1 reachability evidence gates are active: verifier-owned scope classes and observer evidence binding landed with tests; real hosted external-host evidence is still pending as the remaining v13\.1 exit criterion/,
  );
  assert.match(
    status,
    /`external-host` requires a globally routable literal-IP listen host; hostname listen hosts are out of scope for this slice/,
  );
  assert.match(boundary, /状态：active/);
  assert.match(
    boundary,
    /It is active, not complete: the mechanism and negative gates are in place, but the hosted external-host observer run has not happened yet and remains the remaining v13\.1 exit criterion/,
  );
  assert.match(
    boundary,
  ...
  assert.match(
    boundary,
    /Remaining exit criterion: run the observer from a real hosted external host against a globally routable literal-IP listener/,
  );
  const oldObserverField = new RegExp("external_" + "observer_zid");
  for (const doc of [roadmap, draft, status, boundary])
    assert.doesNotMatch(doc, oldObserverField);
});
...
test("v13.2 public docs describe release trust without supply-chain overclaims", async () => {
  ...
  const formatNonClaims =
    /`asp-release-trust\/v1`.*not CycloneDX, not SPDX, not SLSA provenance, not npm registry signing, not package publish, not release transparency,.*not a generic supply-chain platform/s;
  for (const doc of [roadmap, draft, status, boundary])
    assert.match(doc, formatNonClaims);
  ...
  assert.match(
    draft,
    /Release trust staleness means `package_proof_digest` no longer matches the verified referenced package proof/,
  );
  assert.match(
    draft,
    /Trusted release signer pinning applies to the release signer only\. It does not pin or replace the embedded package proof signer trust decision/,
  );
  assert.match(status, /状态：v14 active at v14\.11/);
  assert.match(status, /当前代码基线：`v14\.11-phase-c-local-foundations`/);
  assert.match(
    status,
    /asp-verify\.mjs release-trust <release-trust\.json> \[trusted-release-signers\.json\] verifies `asp-release-trust\/v1` release trust manifests/,
  );
  assert.match(
    status,
    /scripts\/release-trust\.mjs verifies the existing package proof first/,
  );
  assert.match(boundary, /状态：complete/);
  assert.match(
    boundary,
...
test("v13.4 public docs describe evidence-first semantic discovery without marketplace overclaims", async () => {
  ...
  assert.match(
    draft,
    /`FED_QUERY` may carry an `intent` string for semantic discovery/,
  );
  assert.match(
    draft,
    /Ranking is deterministic and evidence-first: exact capability match, trusted capability credential, signed credential claims, audit-backed reputation, zone-chain provenance, verifier-owned routing signals, and semantic token overlap are exposed as inspectable evidence/,
  );
  assert.match(status, /状态：v14 active at v14\.11/);
  assert.match(status, /当前代码基线：`v14\.11-phase-c-local-foundations`/);
  assert.match(
    status,
    /Node federation gateway evidence-first semantic discovery\/reputation ranking/,
  );
  assert.match(boundary, /状态：complete/);
  assert.match(
    boundary,
    /Land the first evidence-first semantic discovery\/reputation ranking primitive without adding vector infrastructure/,
  );
  assert.match(
    boundary,
  ...
  assert.match(
    boundary,
    /not global reputation, not vector-only routing, not public marketplace, and not Go query parity/,
  );
});
...
test("v13.5 public docs describe scheduler-owned ready-DAG Swarm without orchestration overclaims", async () => {
  ...
  assert.match(
    draft,
    /`FED_SWARM_SCHEDULE` accepts a signed Swarm DAG and executes steps in deterministic ready order/,
  );
  assert.match(
    draft,
    /The close proof may include signed scheduler evidence with `mode: "ready-dag"` and the executed `step_order`/,
  );
  assert.match(status, /状态：v14 active at v14\.11/);
  assert.match(status, /当前代码基线：`v14\.11-phase-c-local-foundations`/);
  assert.match(
    status,
    /Go `FED_SWARM_SCHEDULE` scheduler-owned ready-DAG execution/,
  );
  assert.match(boundary, /状态：complete/);
  assert.match(
    boundary,
    /Land the first scheduler-owned ready-DAG Swarm execution primitive/,
  );
  assert.match(
    boundary,
  ...
  assert.match(
    boundary,
    /not automatic decomposition, not parallel execution, not upper-layer master-agent orchestration, and not economic settlement/,
  );
});
...
test("v13.6 public docs describe sandbox proof verification without attestation overclaims", async () => {
  ...
  assert.match(
    boundary,
    /not hardware remote attestation, not container namespace execution, and not a VM\/TEE claim/,
  );
});
...
test("v13.7 public docs describe signed sandbox attestation without hardware overclaims", async () => {
  ...
  assert.match(
    roadmap,
    /Signed sandbox attestation evidence is not hardware remote attestation by itself/,
  );
  assert.match(draft, /local-first prototype at `v14\.10-protocol`/);
  assert.match(
    draft,
  ...
  assert.match(
    boundary,
    /It verifies `asp-sandbox-attestation\/v1` signed evidence/,
  );
  assert.match(
    boundary,
    /not hardware remote attestation, not container namespace execution, and not a TEE quote/,
  );
});
...
test("v13.8 public docs describe pinned external observer identity without hosted overclaim", async () => {
  ...
  for (const doc of [roadmap, draft, status, boundary]) {
    assert.match(
      doc,
      /real hosted external-host observer run (?:is still pending|has not happened yet)/,
    );
  }
});
...
test("v13.11 public docs record audit-backed receipt-count reputation without oracle overclaims", async () => {
  ...
  for (const doc of [roadmap, boundary, status, draft]) {
    ...
    assert.match(
      doc,
      /not a hardcoded demo value, not cross-session (?:ML|learned scoring), (?:and )?not (?:a )?(?:global reputation oracle|third-party reputation service)/,
    );
...
test("v13.13 public docs record zone revocation in discovery without oracle overclaims", async () => {
  ...
  for (const doc of [roadmap, boundary, status, draft]) {
    ...
    assert.match(
      doc,
      /(?:revoked workers lose credential contribution in `agent_score`|Revoked workers get no credential score boost)/,
    );
...
test("v13.15 public docs record Node receipt checkpoint verification without restore or orchestration overclaims", async () => {
  ...
  assert.match(
    draft,
    /Node `FED_RECEIPT` verification requires `checkpoint_refs` and `checkpoints` to have equal length/,
  );
});
...
test("v14 public docs open the overlay/swarm/routing milestone without terminal overclaims", async () => {
  ...
  assert.match(roadmap, /状态：active at v14\.11 \/ Phase C/);
  ...
  assert.match(
    roadmap,
    /candidates sign cost\+latency\+capability micro-contract/,
  );
...
test("v14.7 public docs record policy and risk routing signals", async () => {
  const [roadmap, boundary, status, draft, reputation] = await Promise.all([
    readFile("docs/v14-roadmap.md", "utf8"),
  ...
  for (const doc of [roadmap, boundary, status, draft, reputation]) {
  ...
  assert.match(
    reputation,
    /Missing cost, latency, availability, policy, or risk evidence stays neutral/,
  );
...
test("v14.8 public docs record deterministic Swarm conflict resolution without merge or settlement overclaims", async () => {
  const [roadmap, boundary, status, draft, v149Boundary, federation] =
    await Promise.all([
      readFile("docs/v14-roadmap.md", "utf8"),
  ...
  const crossNetnsDocs = [roadmap, v149Boundary, status, draft, federation];
  ...
  for (const doc of [roadmap, boundary, status, draft]) {
    ...
    assert.match(
      doc,
      /No automatic merge of conflicting content|not automatic merge of conflicting content/i,
    );
  ...
  for (const doc of [v149Boundary, status, draft]) {
    ...
    assert.match(
      doc,
      /not public reachability|not public or hosted external-host reachability|not public/i,
    );
  ...
  assert.match(
    status,
    /The current protocol baseline remains `v14\.11-phase-c-local-foundations`/,
  );
  assert.match(
    status,
    /reachability scope classes \(`local-interface`, `container-observer`, `cross-netns`, `external-host`\)/,
  );
  assert.match(draft, /cross-netns.*literal private inter-namespace IP/);
  assert.match(
    federation,
    /`cross-netns` for trusted separate-network-namespace evidence over a literal private IP/,
  );
...
test("v14.10 public docs record Node ready-DAG parity without orchestration overclaims", async () => {
  const [roadmap, boundary, status, draft, changelog] = await Promise.all([
    readFile("docs/v14-roadmap.md", "utf8"),
...
test("Phase C U19-U30 durable Swarm documentation preserves authority and proof boundaries", async () => {
  const [status, draft, protocol, architecture, changelog] = await Promise.all([
    readFile("docs/implementation-status.md", "utf8"),
    readFile("docs/asp-core-draft.md", "utf8"),
  ...
  const docs = [status, draft, protocol, architecture, changelog];
  for (const doc of docs) {
    ...
    assert.match(
      doc,
      /fenced signed receipt commitment.*exactly-once|exactly-once.*fenced signed receipt commitment/,
    );
    ...
    assert.match(
      doc,
      /real container smoke, cross-host operation, remote artifact handling, or exactly-once worker execution/,
    );
  }
  assert.match(
    status,
    /journal\/view replacement.*close\/disband append faults.*receipt synchronization before response.*stale-lease rejection after reclaim.*concurrent-coordinator exclusion.*ready-wave barriers/,
  );
  assert.match(
    draft,
    /replayable views are materialized from validated replay rather than used as authority/,
  );
  assert.match(changelog, /\[Phase C U19-U30\] - 2026-07-12/);
});

test("current docs distinguish the Phase C local proof kernel from the Ultimate product", async () => {
  const [status, roadmap, vision, architecture, changelog] = await Promise.all([
    readFile("docs/implementation-status.md", "utf8"),
    readFile("docs/v14-roadmap.md", "utf8"),
    readFile("docs/agent-space-ultimate-vision.md", "utf8"),
    readFile("docs/manual/architecture.md", "utf8"),
    readFile("docs/CHANGELOG.md", "utf8"),
  ]);

  for (const doc of [status, roadmap, architecture]) {
    assert.match(doc, /v14\.11/);
    assert.match(doc, /Phase C/);
    assert.match(doc, /Node is a pure verifier of fixed offline U29 vectors/);
    assert.match(doc, /Live public proof excludes durable Swarm completion/);
  }

  assert.doesNotMatch(roadmap, /状态：active at v14\.10/);
  assert.match(status, /## 距离 Ultimate/);
  for (const layer of [
    "Overlay",
    "Discovery",
    "Task Fabric",
  ...
  assert.match(vision, /30-unit local foundation slice is complete/);
  assert.match(vision, /\[INFERENCE\] full Ultimate is ~55-65%/);
  for (const program of [
    "Public overlay and reachability",
    "Distributed discovery and reputation",
    "Remote task fabric and artifact plane",
    "Cross-host Swarm coordination",
    "Hardware-backed trust and isolation",
    "Knowledge network",
    "Semantic OS and human governance",
    "Economy, governance, and production operations",
  ]) {
    assert.match(
      vision,
      new RegExp(program.replace(/[.*+?^${}()|[\\]\\\\]/g, "\\$&")),
    );
  }
  assert.match(architecture, /## Remaining layers/);
  assert.match(changelog, /\[Ultimate gap alignment\] - 2026-07-13/);
  assert.match(changelog, /\[Phase C U19-U30\] - 2026-07-12/);
});
